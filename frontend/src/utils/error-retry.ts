...
function classifyError(err?: unknown): ErrorClassification {
    ...
    if (err instanceof Error && (err.message.includes('timed out') || err.message.includes('timeout'))) {
        return {
...
export function renderErrorWithRetry(
    ...
    const retryHint = retryCount >= 2
        ? `<p class="mt-1 text-xs text-slate-400" data-i18n="error_retry_hint">${t('error_retry_hint', 'Still having trouble? Try refreshing the page.')}</p>`
        : '';
