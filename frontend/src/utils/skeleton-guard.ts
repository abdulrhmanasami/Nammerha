...
export function guardSkeleton(config: SkeletonGuardConfig): () => void {
  const { timeoutMs = 35000, onRetry, messageKey = 'skeleton_timeout' } = config;
  ...
  const earlyTimerId = addTrackedTimer(
    setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      container.innerHTML = `
            <div class="text-center py-8 animate-fade-in-up">
                ...
                ${onRetry ? `<button type="button" class="nm-skeleton-retry mt-3 px-4 py-2 bg-trust-blue text-white text-xs font-bold rounded-lg hover:bg-blue-700 transition-colors">${esc(retryLabel)}</button>` : ''}
            </div>
        `;
