...
const BADGE_SELECTORS =
  '#notif-count, #nav-notif-badge, #mobile-notif-badge, .notif-count, .cart-badge[id$="notif-badge"], [data-notif-badge]';
...
const FOCUSABLE_SELECTOR =
  'button:not([disabled]), [href]:not([tabindex="-1"]), input:not([disabled]), [tabindex]:not([tabindex="-1"])';
...
async function updateBadge(): Promise<void> {
  try {
    ...
    reportWarning('[NotificationPanel] Badge update failed', {
...
async function openPanel(bell: HTMLElement): Promise<void> {
  ...
  if (!panel) {
    ...
    panel.setAttribute('aria-label', t('notifications', 'الإشعارات'));
  ...
  panel.querySelector('#nm-mark-all-read')?.addEventListener('click', async () => {
    try {
      ...
      reportWarning('[NotificationPanel] Mark-all-read failed', {
        component: 'notification-panel',
  ...
  if (generation !== openGeneration || !isOpen || !panel) {
    return;
  }
    ...
    body.querySelectorAll('[data-mark-read]').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        try {
          ...
          // PLT-UX-AUD P3-LOG-006 FIX: Structured telemetry replaces /* silent */.
          reportWarning('[NotificationPanel] Mark-as-read failed', {
            component: 'notification-panel',
            action: 'mark_read',
            notificationId: id,
            error: err instanceof Error ? err.message : String(err),
