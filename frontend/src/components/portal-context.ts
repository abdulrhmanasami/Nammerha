...
function prefetchPortals(currentPortalId: string): void {
    for (const portal of PORTAL_REGISTRY) {
        if (portal.id === currentPortalId) { continue; }
...
function renderContextSwitcher(currentPortalId: string): string {
    const links = PORTAL_REGISTRY
        .filter(p => p.id !== currentPortalId)
        .map(portal => {
    ...
    return `
    <div class="nm-ctx-switcher" role="navigation" aria-label="${t('ctx_switch_portals', 'Switch portals')}">
        <p class="nm-ctx-header" data-i18n="ctx_other_workspaces">${t('ctx_other_workspaces', 'Other Workspaces')}</p>
...
function renderHubSheet(currentPortalId: string): string {
    const cards = PORTAL_REGISTRY.map(portal => {
        ...
        const currentBadge = isCurrent
            ? `<span class="nm-hub-badge" data-i18n="ctx_current">${t('ctx_current', 'Current')}</span>`
            : '';
    ...
    <div class="nm-hub-header">
        <h2 class="nm-hub-title" data-i18n="ctx_workspaces">${t('ctx_workspaces', 'Workspaces')}</h2>
        <button type="button" class="nm-hub-close" aria-label="${t('common_close', 'Close')}">
...
export function mountHubFAB(currentPortalId: string): void {
    ...
    sheetContent.addEventListener('touchstart', (e: TouchEvent) => {
    ...
    sheetContent.addEventListener('touchmove', (e: TouchEvent) => {
        const touch = e.touches[0];
        if (!touch) { return; }
        ...
        if (touchDeltaY > 0) {
            ...
            if (inner) {
                (inner as HTMLElement).style.transform = `translateY(${Math.min(touchDeltaY * 0.5, 100)}px)`;
            }
        }
    }, { passive: true });
...
export function mountContextSwitcher(): void {
    ...
    if (!sidebarHeader || !tabNav) { return; }
    ...
    if ('requestIdleCallback' in window) {
        (window as unknown as { requestIdleCallback: (cb: () => void) => void })
            .requestIdleCallback(() => prefetchPortals(currentPortalId));
    } else {
