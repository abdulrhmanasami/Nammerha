 ...
 */
export async function loadProjectMarkers(map: maplibregl.Map, apiBase = '/api'): Promise<void> {
  try {
    ...
    map.addLayer({
      ...
      paint: {
        'circle-color': [
          ...
          ['get', 'status'],
          'published',
          STATUS_COLORS['published'] as string,
          'in_progress',
          STATUS_COLORS['in_progress'] as string,
          'completed',
          STATUS_COLORS['completed'] as string,
    ...
    reportError(
      error instanceof Error ? error : new Error('[Nammerha Map] Failed to load project markers'),
      { component: 'map_markers', action: 'load_markers' },
    );
  }
}
...
function setupClusterInteractions(map: maplibregl.Map): void {
  map.on('click', CLUSTER_LAYER, async (e) => {
    ...
    try {
      ...
      reportError(
        err instanceof Error ? err : new Error('[Nammerha Map] Cluster expansion error'),
        { component: 'map_markers', action: 'cluster_expand' },
      );
    }
  });
...
function getStatusLabel(status: string): string {
  const labels: Record<string, string> = {
    published: t('map_status_needs_funding', '🟡 بحاجة لتمويل'),
    in_progress: t('map_status_in_progress', '🔵 قيد التنفيذ'),
    completed: t('map_status_completed', '🟢 مكتمل'),
  };
 ...
 */
export function getProjectStats(map: maplibregl.Map): {
  total: number;
  inProgress: number;
  funded: number;
  completed: number;
