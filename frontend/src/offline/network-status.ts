...
const STATUS_KEYS: Record<StatusType, { key: string; fb: string }> = {
  offline: {
    key: 'network_offline',
    fb: 'You are offline — changes will sync when connection returns',
  },
...
function showStatus(type: StatusType, autoHideMs?: number, worker?: ServiceWorker): void {
  ...
  if (type === 'swUpdate') {
    const msg = t('network_sw_update', 'إصدار جديد متاح — ');
    ...
    if (!isOnline()) {
      ...
      if (count > 0) {
        ...
        if (bar) {
          ...
          const badgeText = `${count} ${t('network_pending', 'تغييرات معلّقة')}`;
          if (badge) {
