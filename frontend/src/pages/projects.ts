...
function showView(view: 'skeleton' | 'grid' | 'empty' | 'error'): void {
  ...
  if (countEl) {
    ...
    countEl.textContent =
      view === 'skeleton'
        ? t('projects_loading', 'جاري تحميل المشاريع...')
        : `${displayed.length} ${t('projects_count', 'مشاريع')}`;
  }
}
...
function createProjectCard(project: ProjectCard, index: number): HTMLElement {
  ...
  card.className =
    'block glass-card rounded-xl overflow-hidden border border-slate-100 hover:shadow-lg transition-all duration-200 animate-fade-in-up';
  ...
  const config = damageTypeConfig[project.damage_type] ??
    damageTypeConfig['general'] ?? {
      icon: 'ph-hammer',
  ...
  card.innerHTML = `
        ...
        <!-- Content -->
        <div class="p-4">
            <h3 class="text-sm font-bold text-slate-900 mb-1 line-clamp-2 dark:text-slate-100">${esc(project.title)}</h3>
            <p class="text-xs text-slate-400 flex items-center gap-1 mb-3 dark:text-slate-500">
                <i class="ph ph-map-pin" aria-hidden="true"></i>
                ${esc(project.address_text || t('projects_unknown_location', 'موقع غير محدد'))}
            </p>
            <!-- Funding progress -->
      ...
      if (state.hasMore) {
        ...
        if (emptyEl) {
          ...
          if (!hintEl) {
            ...
            hint.className =
              'nm-search-hint text-xs text-trust-blue mt-2 cursor-pointer hover:underline';
            hint.setAttribute('data-i18n', 'projects_load_more_hint');
            hint.textContent = t(
              'projects_load_more_hint',
              'حمّل المزيد من المشاريع لتوسيع نتائج البحث',
            );
