...
function renderKpis(summary: RevenueAdminSummary): void {
  ...
  if (totalEl) {
    totalEl.textContent = formatCents(totalRevenue);
  }
  if (commEl) {
    commEl.textContent = formatCents(summary.total_commission_cents);
  }
  if (commCountEl) {
    commCountEl.textContent = String(summary.commission_count);
  }
  if (tipEl) {
    tipEl.textContent = formatCents(summary.total_tip_cents);
  }
  if (tipCountEl) {
    tipCountEl.textContent = String(summary.tip_count);
  }
  if (avgTipEl) {
    avgTipEl.textContent = formatCents(summary.avg_tip_cents);
  }
  if (avgTipPctEl) {
  ...
  if (tiers.length === 0) {
    tbody.innerHTML = `
        <div class="bg-white py-8 text-center w-full nm-table-empty dark:bg-dark-surface">
            ...
            <p class="font-bold text-slate-700 text-sm mt-2 dark:text-slate-300">${escapeHtml(t('rev_no_tiers', 'لا توجد مستويات'))}</p>
        </div>`;
    return;
  }
  ...
  tbody.innerHTML = tiers
    .map((tier) => {
      ...
      const statusText = tier.is_active
        ? escapeHtml(t('rev_active', 'نشط'))
        : escapeHtml(t('rev_inactive', 'غير نشط'));
...
function renderRecentCommissions(commissions: CommissionEntry[]): void {
  ...
  if (commissions.length === 0) {
    list.innerHTML = `<div class="px-5 py-8 text-center text-sm text-slate-400 dark:text-slate-500">${escapeHtml(t('rev_no_commissions', 'لا توجد عمولات'))}</div>`;
    return;
  }
      ...
      (c) => `
        <div class="px-5 py-3 flex items-center gap-4">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-sm font-medium truncate">${escapeHtml(t('rev_po_label', 'أمر الشراء'))} ${escapeHtml(c.po_id)} — ${formatCents(c.commission_amount_cents)}</p>
                <p class="text-3xs text-slate-400 mt-0.5 dark:text-slate-500">${escapeHtml(t('rev_rate_label', 'النسبة'))}: ${bpsToPercent(c.rate_bps)} • ${escapeHtml(t('rev_po_label', 'أمر الشراء'))}: ${formatCents(c.po_amount_cents)}</p>
            </div>
...
export function renderRecentTips(tips: TipEntry[]): void {
  ...
  if (tips.length === 0) {
    list.innerHTML = `<div class="px-5 py-8 text-center text-sm text-slate-400 dark:text-slate-500">${escapeHtml(t('rev_no_tips', 'لا توجد إكراميات'))}</div>`;
    return;
  }
...
async function loadDashboard(): Promise<void> {
  ...
  try {
    ...
    if (
      tiersSettled.status === 'fulfilled' &&
      tiersSettled.value.success &&
      tiersSettled.value.data
    ) {
    ...
    if (
      commissionsSettled.status === 'fulfilled' &&
      commissionsSettled.value.success &&
      commissionsSettled.value.data &&
      'rows' in commissionsSettled.value.data
    ) {
      renderRecentCommissions(commissionsSettled.value.data.rows);
    } else if (commissionsSettled.status === 'rejected') {
      reportWarning('[Revenue] Commissions API failed', {
        error: String(commissionsSettled.reason),
      });
    }
  } catch (err) {
    ...
    const tbody = document.getElementById('tier-table-body');
    if (tbody) {
      renderErrorWithRetry(tbody, loadDashboard, undefined, undefined, err);
    }
    const commList = document.getElementById('recent-commissions-list');
    if (commList) {
      renderErrorWithRetry(commList, loadDashboard, undefined, undefined, err);
    }
  }
