...
async function initHomepageMap(): Promise<void> {
  ...
  if (!container) {
    reportWarning('#main-map container not found — skipping map init', {
  ...
  try {
    ...
    map.on('load', async () => {
      ...
      try {
        ...
        reportWarning('Map data load failed', {
          component: 'homepage_map',
          action: 'load_markers',
          error: String(err),
        });
        // W13-004 FIX: Show user-facing feedback on marker load failure.
        showToast(t('map_data_error', 'فشل تحميل بيانات الخريطة'));
      }
    });
    ...
    reportWarning('Map initialization failed', {
      component: 'homepage_map',
      action: 'init_map',
      error: String(err),
    });
    ...
    if (container) {
      container.innerHTML = `<div class="flex flex-col items-center justify-center h-full text-slate-400 p-8 dark:text-slate-500">
                ...
                <p class="mt-2 text-sm font-medium">${esc(t('map_init_error', 'فشل تهيئة الخريطة'))}</p>
            </div>`;
    }
  }
}
...
function updateStatsOverlay(map: maplibregl.Map): void {
  ...
  if (countEl) {
    ...
    countEl.textContent = tParams('map_active_projects_count', `${total} Active Projects`, {
      count: total,
    });
  }
}
...
function getRegionName(lat: number, lng: number, zoom: number): string {
  if (zoom < 8) {
    return t('map_region_syria', 'سوريا');
  }
  ...
  const cities: { i18nKey: string; fallback: string; lat: number; lng: number }[] = [
    ...
    { i18nKey: 'map_region_aleppo', fallback: 'Aleppo', lat: 36.2, lng: 37.15 },
    ...
    { i18nKey: 'map_region_daraa', fallback: 'Daraa', lat: 32.63, lng: 36.1 },
    { i18nKey: 'map_region_idlib', fallback: 'Idlib', lat: 35.93, lng: 36.63 },
    { i18nKey: 'map_region_hasakah', fallback: 'Al-Hasakah', lat: 36.5, lng: 40.74 },
  ];
