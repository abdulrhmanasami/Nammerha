    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        showSimpleBanner(
          'dashboard-banner',
          'error',
          t('tp_availability_error', 'فشل تحديث الحالة. حاول مرة أخرى.'),
        );
      }
    });
  });
...
function updateAvailabilityUI(status: string): void {
  ...
  container.querySelectorAll('button').forEach((btn) => {
    ...
    if (s === status) {
      ...
      btn.className =
        'flex-1 px-2 py-1.5 text-3xs font-bold rounded-lg border border-slate-200 text-slate-500';
    }
  });
...
async function loadStats(): Promise<void> {
  try {
    ...
    const [statsRes, profileRes] = await Promise.allSettled([
      swrFetch('tp-stats', () => tradesperson.getStats(), {
        maxAge: 120_000,
        onStaleData: () => {
          showStaleIndicator();
        },
    ...
    if (statsRes.status === 'fulfilled' && statsRes.value.data) {
      ...
      if (ratingEl) {
        ratingEl.innerHTML = s.average_rating
          ? `${s.average_rating.toFixed(1)} <i class="ph ph-star nm-star-rating nm-icon-gap-start" aria-hidden="true"></i>`
          : '—';
      }
      setText('pending-count', String(s.pending_requests));
    } else {
      ['kpi-active', 'kpi-completed', 'kpi-earnings', 'kpi-rating'].forEach((id) =>
        setText(id, '—'),
      );
    ...
    reportWarning('[Tradesperson] Stats load failed, showing defaults', {
      component: 'tradesperson',
      action: 'load_stats',
  ...
  try {
    ...
    if (requests.length === 0 && assignments.length === 0) {
      tbody.innerHTML = renderEmptyState({
        icon: 'sun-dim',
        title: t('tp_no_active_work', 'لا يوجد عمل نشط'),
        subtitle: t('tp_check_available', 'تحقق من الأعمال المتاحة لفرص جديدة'),
      });
      return;
    }
    ...
    if (hasRequests) {
      sectionsHtml += `
                <div class="flex items-center gap-2 mb-3 mt-1">
                    ...
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400" data-i18n="tp_direct_requests">${esc(t('tp_direct_requests', 'طلبات مباشرة'))}</h3>
    ...
    if (hasAssignments) {
      sectionsHtml += `
                <div class="flex items-center gap-2 mb-3 ${hasRequests ? 'mt-6 pt-4 border-t border-slate-200 dark:border-dark-border' : 'mt-1'}">
                    ...
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400" data-i18n="tp_contractor_assignments">${esc(t('tp_contractor_assignments', 'مهام المقاول'))}</h3>
  ...
  try {
    ...
    renderProgressive({
            ...
            <div class="p-5 hover:bg-slate-50/50 transition-colors animate-fade-in-up" style="animation-delay:${staggerDelay(i)}">
                <div class="flex items-start justify-between gap-4">
                    <div class="flex-1">
                        ...
                        <p class="text-xs text-slate-500 mt-1 dark:text-slate-400">${esc(r.description ?? t('tp_no_description', 'بدون وصف'))}</p>
      ...
      emptyState: () =>
        renderEmptyState({
          ...
          title: t('tp_no_requests', 'لا توجد طلبات مطابقة لمهنتك'),
          subtitle: t('tp_new_requests_auto', 'ستظهر الطلبات الجديدة هنا تلقائياً'),
        }),
    });
    ...
    if (!delegationWired.requests) {
      ...
      container.addEventListener('click', async (e: MouseEvent) => {
        ...
        btn.disabled = true;
        btn.textContent = t('tp_accepting', 'جاري القبول...');
        ...
        try {
          ...
          btn.innerHTML = `<i class="ph ph-check nm-icon-gap-end" aria-hidden="true"></i>${esc(t('tp_accepted', '✓ تم القبول'))}`;
          btn.setAttribute('data-i18n', 'tp_accepted');
          btn.className =
            'px-4 py-2 bg-green-100 text-green-700 text-xs font-bold rounded-lg shrink-0';
          loadStats();
        } catch (err) {
    ...
    reportError(err instanceof Error ? err : new Error('[Tradesperson] Requests load failed'), {
      component: 'tradesperson',
  ...
  try {
    ...
    renderProgressive({
      ...
      emptyState: () =>
        renderEmptyState({
          icon: 'clipboard-text',
          title: t('tp_no_assignments', 'لا توجد تكليفات من المقاولين'),
        }),
    });
    ...
    if (!delegationWired.assignments) {
      ...
      tbody.addEventListener('click', async (e: MouseEvent) => {
        ...
        try {
          ...
          reportError(
            err instanceof Error ? err : new Error('[Tradesperson] Assignment response failed'),
            { component: 'tradesperson', action: 'respond_assignment' },
          );
          showSimpleBanner(
            'dashboard-banner',
            'error',
            t('tp_response_error', 'فشل الإجراء. حاول مرة أخرى.'),
          );
        }
      });
    }
  } catch (err) {
    reportError(err instanceof Error ? err : new Error('[Tradesperson] Assignments load failed'), {
      component: 'tradesperson',
  ...
  try {
    ...
    renderProgressive({
      ...
      emptyState: () =>
        renderEmptyState({
          icon: 'coins',
          title: t('tp_no_earnings', 'لا توجد أرباح بعد'),
        }),
    });
  } catch (err) {
    reportError(err instanceof Error ? err : new Error('[Tradesperson] Earnings load failed'), {
      component: 'tradesperson',
      action: 'load_earnings',
    });
