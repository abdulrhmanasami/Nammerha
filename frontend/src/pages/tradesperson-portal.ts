                ...
                <div class="flex items-start justify-between gap-4">
                    <div class="flex-1">
                        ...
                        <div class="flex flex-wrap items-center gap-3 mt-2 text-[10px] text-slate-400">
                            ...
                            ${r.budget_max ? `<span><i class="ph ph-coins" aria-hidden="true"></i> <span data-i18n="tp_budget">Budget</span>: $${(r.budget_max / 100).toLocaleString()}</span>` : ''}
    ...
    try {
        ...
        container.innerHTML = `
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div><p class="text-[10px] font-bold text-slate-400 uppercase" data-i18n="Hourly Rate">Hourly Rate</p><p class="font-medium mt-0.5">${p.hourly_rate ? `$${(p.hourly_rate / 100).toFixed(2)}/hr` : '—'}</p></div>
                <div><p class="text-[10px] font-bold text-slate-400 uppercase" data-i18n="Daily Rate">Daily Rate</p><p class="font-medium mt-0.5">${p.daily_rate ? `$${(p.daily_rate / 100).toFixed(2)}/day` : '—'}</p></div>
...
/**
 * P2-FE-003 FIX: Locale-aware time-ago helper.
 * Uses data-i18n spans so the i18n engine handles locale switching.
 */
function timeAgo(dateStr: string): string {
    const diff = Date.now() - new Date(dateStr).getTime();
    const hours = Math.floor(diff / 3600000);
    if (hours < 1) { return `<span data-i18n="tp_just_now">Just now</span>`; }
    if (hours < 24) { return `${hours} <span data-i18n="tp_hours_ago">h ago</span>`; }
    return `${Math.floor(hours / 24)} <span data-i18n="tp_days_ago">d ago</span>`;
}
