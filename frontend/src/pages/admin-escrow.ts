      ...
      if (panel) {
        panel.innerHTML = `
                    <div class="col-span-2 flex flex-col items-center justify-center py-16 text-center">
                        ...
                        <p class="text-lg font-bold text-slate-600 dark:text-slate-400">${esc(t('esc_all_cleared', 'تم تصفية الكل'))}</p>
                        <p class="text-sm text-slate-400 mt-1 dark:text-slate-500">${esc(t('esc_no_pending', 'لا توجد معاملات معلقة'))}</p>
                    </div>
                `;
      }
      // Disable nav buttons
...
function renderCase(index: number): void {
  ...
  if (resolvedCases.has(index) || c.status === 'released' || c.status === 'flagged') {
    ...
    if (flagBtn) {
      ...
      flagBtn.innerHTML = `<i class="ph ph-warning-diamond text-lg" aria-hidden="true"></i> ${esc(t('esc_flag_discrepancy', 'الإبلاغ عن تناقض'))}`;
...
function gpsAccuracyLabel(meters: number): string {
  if (meters <= 2) {
    return `${t('esc_signal_high', 'إشارة عالية')} (${t('esc_accuracy', 'الدقة')} ${meters.toFixed(1)}m)`;
  }
  if (meters <= 5) {
    return `${t('esc_signal_medium', 'إشارة متوسطة')} (${t('esc_accuracy', 'الدقة')} ${meters.toFixed(1)}m)`;
  }
  return `${t('esc_signal_low', 'إشارة منخفضة')} (${t('esc_accuracy', 'الدقة')} ${meters.toFixed(1)}m)`;
}
...
function initActionButtons(): void {
  ...
  if (releaseBtn) {
    releaseBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        releaseBtn.classList.add('bg-trust-blue');
        releaseBtn.innerHTML = `<i class="ph ph-check-circle text-lg" aria-hidden="true"></i> ${esc(t('esc_release_funds', 'تم التحقق: تحرير الأموال للمورد'))}`;
        const msg = err instanceof Error ? err.message : t('esc_release_error', 'فشل الإفراج');
        showToast(msg, 'error');
      }
    });
  }
  ...
  if (flagBtn) {
    ...
    flagBtn.addEventListener('click', async () => {
      ...
      if (reason === '') {
        showToast(t('esc_reason_required', 'السبب مطلوب'));
        flagInput?.focus();
        return;
      }
      ...
      // Loading state
      flagBtn.disabled = true;
      flagBtn.innerHTML = `<i class="ph ph-spinner animate-spin text-lg" aria-hidden="true"></i> ${esc(t('esc_flagging', 'جاري الإبلاغ…'))}`;
      ...
      try {
        ...
        flagBtn.innerHTML = `<i class="ph ph-flag text-lg" aria-hidden="true"></i> ${esc(t('esc_discrepancy_flagged', '⚠ تم الإبلاغ عن تناقض'))}`;
        ...
        showToast(
          `${esc(t('esc_flagged_toast', 'تم الإبلاغ عن المعاملة'))}: "${esc(reason)}" — ${esc(t('esc_under_investigation', 'قيد التحقيق'))}`,
          'success',
        );
      } catch (err) {
        // Re-enable on failure — admin must be able to retry.
        flagBtn.disabled = false;
        flagBtn.innerHTML = `<i class="ph ph-flag text-lg" aria-hidden="true"></i> ${esc(t('esc_submit_flag', 'إبلاغ'))}`;
        const msg = err instanceof Error ? err.message : t('esc_flag_error', 'فشل الإبلاغ');
        showToast(msg, 'error');
      }
    });
  }
