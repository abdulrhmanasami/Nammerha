...
const itemsContainer = document.querySelector(
  '.flex.flex-col.gap-3.px-4.pb-32',
) as HTMLElement | null;
...
function renderItem(item: BOQItem, index: number): string {
  ...
  const oracleDisplay = item.oracle_price
    ? `${t('boq_oracle', 'المؤشر')}: ${formatCents(item.oracle_price)}/${item.unit}`
    : t('boq_no_oracle_price', 'لا يوجد سعر مرجعي');
      ...
      <div class="flex flex-1 flex-col justify-between">
        ...
        <div class="flex items-center justify-between mt-2">
          <div class="flex flex-col">
            <p class="text-slate-400 text-3xs uppercase font-bold tracking-tighter dark:text-slate-500">${esc(t('boq_estimated', 'مُقدّر'))}</p>
  ...
  if (state.items.length === 0) {
    itemsContainer.innerHTML = `
        <div class="text-center py-16">
          ...
          <p class="text-slate-500 font-bold mt-4 dark:text-slate-400">${esc(t('boq_no_materials', 'لم تُضف مواد بعد'))}</p>
          <p class="text-slate-400 text-sm mt-1 dark:text-slate-500">${esc(t('boq_search_hint', 'ابحث عن مواد أعلاه لبناء جدول الكميات'))}</p>
        </div>`;
    return;
  }
...
async function loadExistingBOQ(): Promise<void> {
  ...
  try {
    ...
    reportError(err instanceof Error ? err : new Error('[BOQ] Failed to load existing items'), {
      component: 'engineer_boq',
...
materialSearch?.addEventListener('input', () => {
  ...
  searchTimeout = addTrackedTimer(setTimeout(async () => {
    ...
    try {
      ...
      reportError(err instanceof Error ? err : new Error('[BOQ] Oracle search failed'), {
        component: 'engineer_boq',
        action: 'oracle_search',
      });
...
publishBtn?.addEventListener('click', async () => {
  if (state.isPublishing || !state.projectId || state.items.length === 0) {
    return;
  }
  ...
  if (publishBtn) {
    ...
    publishBtn.innerHTML = `<i class="ph ph-spinner ph-lg animate-spin" aria-hidden="true"></i> ${esc(t('boq_publishing', 'جاري النشر...'))}`;
  }
  ...
  try {
    ...
    if (failures.length > 0) {
      ...
      if (failures.length === state.items.length) {
        ...
        throw new Error(t('boq_all_items_failed', 'فشل نشر جميع البنود'));
      }
    }
    ...
    if (publishBtn) {
      publishBtn.innerHTML = `<i class="ph ph-check-circle" aria-hidden="true"></i> ${esc(t('boq_published', 'تم النشر!'))}`;
    ...
    if (publishBtn) {
      ...
      publishBtn.innerHTML = `<i class="ph ph-upload" aria-hidden="true"></i> ${esc(t('boq_publish_to_marketplace', 'نشر في السوق'))}`;
    }
  } finally {
