...
function setupTabs(): void {
  ...
  document.getElementById('tab-captures')?.addEventListener('click', () => {
    haptic.light();
    switchTab('captures');
  });
...
async function loadKPIs(): Promise<void> {
  try {
    ...
    reportWarning('[EngineerPortal] KPI load failed', {
      error: err instanceof Error ? err.message : String(err),
    });
    [
      'kpi-assigned-projects',
      'kpi-proofs-pending',
      'kpi-proofs-verified',
      'kpi-escrow-released',
    ].forEach((id) => {
      ...
      renderItem: (p, i) => `
            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm hover:shadow-md transition-shadow dark:bg-dark-surface dark:border-dark-border animate-fade-in-up" style="animation-delay:${staggerDelay(i)}">
                ...
                <div class="flex items-center gap-4 text-xs text-slate-500 mb-3 dark:text-slate-400">
                    <span class="flex items-center gap-1"><i class="ph ph-map-pin text-sm" aria-hidden="true"></i> ${esc(p.region || t('eng_no_region', 'غير محدد'))}</span>
                    <span class="flex items-center gap-1"><i class="ph ph-list-checks text-sm" aria-hidden="true"></i> ${esc(String(p.boq_count))} ${esc(t('eng_boq_items', 'بنود الكميات'))}</span>
                </div>
                ...
                <div class="mt-3 flex gap-2">
                    <a href="engineer-boq.html?project=${p.project_id}" class="text-3xs font-bold text-trust-blue hover:underline flex items-center gap-1">
                        <i class="ph ph-list-checks" aria-hidden="true"></i> ${esc(t('eng_view_boq', 'عرض جدول الكميات'))}
                    </a>
                    <a href="engineer-camera.html?project=${p.project_id}" class="text-3xs font-bold text-smoky-jade hover:underline flex items-center gap-1">
                        <i class="ph ph-camera" aria-hidden="true"></i> ${esc(t('eng_capture', 'التقاط'))}
                    </a>
                </div>
            </div>`,
      emptyState: () =>
        renderEmptyState({
          icon: 'buildings',
          title: t('eng_no_projects', 'لا توجد مشاريع مسندة بعد'),
          subtitle: t('eng_no_projects_desc', 'ستظهر المشاريع هنا عند إسنادها من المنصة'),
        }),
    });

    applyI18n();
  } catch (err) {
    reportWarning('[EngineerPortal] Projects load failed', {
      error: err instanceof Error ? err.message : String(err),
    });
  ...
  try {
    ...
    renderProgressive({
      ...
      renderItem: (b, i) => `
            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm hover:shadow-md transition-shadow dark:bg-dark-surface dark:border-dark-border animate-fade-in-up" style="animation-delay:${staggerDelay(i)}">
                ...
                <div class="flex items-center justify-between border-t border-slate-100 pt-3 mt-2 dark:border-dark-border">
                    <div>
                        <p class="text-3xs font-bold text-slate-400 uppercase tracking-wider dark:text-slate-500">${esc(t('eng_proposed_cost', 'التكلفة المقترحة'))}</p>
                        <p class="font-mono font-bold text-sm text-smoky-jade dark:text-emerald-400">${formatCents(b.proposed_cost)}</p>
                    </div>
                    <div class="text-center">
                        <p class="text-3xs font-bold text-slate-400 uppercase tracking-wider dark:text-slate-500">${esc(t('eng_est_days', 'المدة التقديرية'))}</p>
                        <p class="font-bold text-sm text-slate-700 dark:text-slate-300">${b.estimated_days}</p>
                    </div>
                    <div class="text-end">
                        <p class="text-3xs font-bold text-slate-400 uppercase tracking-wider dark:text-slate-500">${esc(t('eng_submitted', 'تاريخ التقديم'))}</p>
      ...
      emptyState: () =>
        renderEmptyState({
          icon: 'flag-banner',
          title: t('eng_no_bids', 'لم تقدم أي عروض بعد'),
        }),
    });

    applyI18n();
  } catch (err) {
    reportWarning('[EngineerPortal] Bids load failed', {
      error: err instanceof Error ? err.message : String(err),
    });
  ...
  try {
    ...
    renderProgressive({
      ...
      emptyState: () =>
        renderEmptyState({
          icon: 'camera',
          title: t('eng_no_captures', 'لا توجد التقاطات بعد'),
          subtitle: t('eng_no_captures_desc', 'ابدأ بالتقاط أدلة ميدانية عبر الكاميرا الميدانية'),
        }),
    });
    ...
    container.querySelectorAll<HTMLImageElement>('img[data-capture-fallback]').forEach((img) => {
      img.addEventListener(
        ...
        () => {
          ...
          if (parent) {
            parent.innerHTML =
              '<div class="size-14 flex items-center justify-center"><i class="ph ph-image-broken text-slate-400 text-xl" aria-hidden="true"></i></div>';
          }
        },
        { once: true },
      );
    ...
    reportWarning('[EngineerPortal] Captures load failed', {
      error: err instanceof Error ? err.message : String(err),
    });
...
function phaseColor(phase: string): string {
  switch (phase?.toLowerCase()) {
    ...
    default:
      return 'bg-slate-100 text-slate-600 dark:bg-dark-elevated dark:text-slate-400';
  }
}
...
function phaseLabel(phase: string): string {
  switch (phase?.toLowerCase()) {
    case 'planning':
      return t('eng_phase_planning', 'تخطيط');
    case 'in_progress':
      return t('eng_phase_in_progress', 'قيد التنفيذ');
    case 'construction':
      return t('eng_phase_construction', 'بناء');
    case 'completed':
      return t('eng_phase_completed', 'مكتمل');
    case 'delivered':
      return t('eng_phase_delivered', 'تم التسليم');
    case 'published':
      return t('eng_phase_published', 'منشور');
    default:
      return phase;
  }
}
...
function bidStatusLabel(status: string): string {
  switch (status?.toLowerCase()) {
    case 'pending':
      return t('eng_bid_pending', 'قيد المراجعة');
    case 'accepted':
      return t('eng_bid_accepted', 'مقبول');
    case 'rejected':
      return t('eng_bid_rejected', 'مرفوض');
    default:
      return status;
  }
}
