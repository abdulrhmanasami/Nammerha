...
if ('geolocation' in navigator) {
    navigator.geolocation.watchPosition((pos) => {
        ...
        if (gpsCoordsEl)
            gpsCoordsEl.textContent = '36.2021° N, 37.1342° E';
        if (gpsAccuracyEl)
            gpsAccuracyEl.textContent = 'GPS: Simulated Mode';
...
if (captureBtn) {
    captureBtn.addEventListener('click', () => {
        ...
        if (cameraReady && cameraCaptured) {
            ...
            if (textEl)
                textEl.textContent = `Captured #${captureCount} ✓`;
            // Reset after 2s
            setTimeout(() => {
                ...
                if (textEl)
                    textEl.textContent = 'Capture 360 & Sync';
            }, 2000);
        }
