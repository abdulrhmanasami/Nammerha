...
const TIERS: Record<string, TierPricing> = {
  pro: { monthly: 1500, yearly: Math.round(1500 * 0.8) }, // $15 → $12
  business: { monthly: 4900, yearly: Math.round(4900 * 0.8) }, // $49 → $39
  enterprise: { monthly: 9900, yearly: Math.round(9900 * 0.8) }, // $99 → $79
};
...
function updateAllPrices(): void {
  ...
  intervalEls.forEach((el) => {
    el.textContent = isYearly
      ? t('pricing_per_month_yearly', '/شهر (سنوي)')
      : t('pricing_per_month', '/ شهرياً');
  });
}
...
async function handleSubscribe(planSlug: string): Promise<void> {
  ...
  try {
    ...
    if (body.success) {
      ...
      if (btn) {
        btn.innerHTML = `<i class="ph ph-check nm-icon-gap-end" aria-hidden="true"></i>${escapeHtml(t('pricing_subscribed', 'تم الاشتراك'))}`;
