...
export const revenueAdmin = {
  ...
  getCommissions: (limit = 8) =>
    request<{ rows: CommissionEntry[]; total: number }>(
...
export const storage = {
  presign: (data: { filename: string; content_type: string; purpose?: string }) =>
    request<PresignResponse>('/storage/presign', { method: 'POST', body: JSON.stringify(data) }),
  getUploadUrl: (data: {
    project_id: string;
    ...
    request<{ upload_url: string; file_key: string; public_url: string; expires_at: string }>(
      '/storage/upload-url',
