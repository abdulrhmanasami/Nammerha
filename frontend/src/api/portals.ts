...
export const tradesperson = {
  ...
  updateAvailability: (status: 'available' | 'busy' | 'offline') =>
    request('/tradesperson/availability', { method: 'PATCH', body: JSON.stringify({ status }) }),
};
...
export const supplier = {
  ...
  getCatalog: (params?: { limit?: number; offset?: number; search?: string }) => {
    ...
    const q = qs.toString();
    return request<CatalogItem[]>(`/supplier/catalog${q ? `?${q}` : ''}`);
  },
  updateCatalogItem: (
    itemId: string,
    data: {
  ...
  reactivateItem: (itemId: string) =>
    request<CatalogItem>(`/supplier/catalog/${itemId}/reactivate`, {
      method: 'POST',
      headers: { 'Idempotency-Key': crypto.randomUUID() },
    }),
  getOrders: (status?: string, params?: { limit?: number; offset?: number }) => {
    const qs = new URLSearchParams();
    if (status) {
    ...
    const q = qs.toString();
    return request<PurchaseOrder[]>(`/supplier/orders${q ? `?${q}` : ''}`);
  },
...
export const engineer = {
  getProjects: (status?: string, params?: { limit?: number; offset?: number }) => {
    const qs = new URLSearchParams();
    if (status) {
      qs.set('status', status);
    }
    ...
    const q = qs.toString();
    return request<EngineerProject[]>(`/engineer/projects${q ? `?${q}` : ''}`);
  },
  getStats: () => request<EngineerStats>('/engineer/stats'),
  getBids: (status?: string, params?: { limit?: number; offset?: number }) => {
    const qs = new URLSearchParams();
    if (status) {
      qs.set('status', status);
    }
    ...
    const q = qs.toString();
    return request<EngineerBid[]>(`/engineer/bids${q ? `?${q}` : ''}`);
  },
  getProfile: () => request<EngineerProfile>('/engineer/profile'),
  getCaptures: (limit?: number) => {
    const qs = limit ? `?limit=${limit}` : '';
    return request<EngineerCapture[]>(`/engineer/captures${qs}`);
  },
  submitCapture: (data: {
    project_id: string;
    file_url: string;
...
export const contractor = {
  getProjects: (status?: string, params?: { limit?: number; offset?: number }) => {
    ...
    const q = qs.toString();
    return request<ContractorProject[]>(`/contractor/projects${q ? `?${q}` : ''}`);
  },
  ...
  getBids: (status?: string, params?: { limit?: number; offset?: number }) => {
    ...
    const q = qs.toString();
    return request<ContractorBid[]>(`/contractor/bids${q ? `?${q}` : ''}`);
  },
  getMarketplace: (params?: { limit?: number; offset?: number }) => {
    ...
    const q = qs.toString();
    return request<AvailableProject[]>(`/contractor/marketplace${q ? `?${q}` : ''}`);
  },
...
export const homeowner = {
  getProjects: (params?: { limit?: number; offset?: number }) => {
    ...
    const q = qs.toString();
    return request<HomeownerProject[]>(`/homeowner/projects${q ? `?${q}` : ''}`);
  },
  ...
  getServiceRequests: (params?: { limit?: number; offset?: number }) => {
    const qs = new URLSearchParams();
    if (params?.limit) {
      qs.set('limit', String(params.limit));
    }
    ...
    const q = qs.toString();
    return request<HomeownerServiceRequest[]>(`/homeowner/service-requests${q ? `?${q}` : ''}`);
  },
  cancelServiceRequest: (requestId: string) =>
    request(`/homeowner/service-requests/${requestId}/cancel`, {
      method: 'POST',
      headers: { 'Idempotency-Key': crypto.randomUUID() },
    }),
...
// ── user Types — ERADICATED ────────────────────────────────────────────────
// P2-003: Entire user API client removed — payment Eradication KI (May 2026).
// Backend endpoints (/user/*) are preserved for future use per KI mandate.
// See: nammerha_payment_suspension KI for full context.
