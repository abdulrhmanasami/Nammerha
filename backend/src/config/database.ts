...
function buildSslConfig(): tls.ConnectionOptions | false {
  ...
  throw new Error(
    `[DB] Unknown sslmode: "${sslMode}". Use disable, require, verify-ca, or verify-full.`,
  );
}
 ...
 *
 * NOTE: For financial/monetary operations (escrow, payments),
...
// Nammerha Domain Law 1: ALL escrow releases and payment mutations
 ...
 */
export async function financialTransaction<T>(fn: (client: PoolClient) => Promise<T>): Promise<T> {
  const MAX_RETRIES = 3;
