// ============================================================================
// Nammerha Backend — Crowdfunding Service (Path 2: User → Escrow)
// ============================================================================
// Handles the itemized micro-funding flow:
//   1. User browses marketplace (published projects on map)
//   2. User selects specific BOQ items to fund
...
import type { ProjectCard, BOQFunding, EscrowLedger, CreateTransactionDTO } from '../types';
...
// ─── Path 2.2: Get Project BOQ (Shopping Basket) ──────────────────────────────

/**
 * Get itemized BOQ with funding progress for the shopping basket UI.
...
// ─── Path 2.3: Create Payment Intent (Escrow Lock) ────────────────────────────────

/**
 * Processes a user's contribution to specific BOQ items.
 * For each item:
...
export async function createPaymentIntent(
  ...
  // With max=10 connections, 10 concurrent payments = total platform freeze.
  //
  ...
  const pendingItems = await transaction(async (client) => {
    ...
    for (const fundItem of dto.items) {
      ...
      // 3. Cap payment at remaining need (prevent over-funding)
      const actualAmount = Math.min(fundItem.amount, remainingNeed);
      if (actualAmount <= 0) {
        throw new Error(`Invalid payment amount for item ${fundItem.item_id}`);
      }

      // 4. Create escrow entry as 'pending' (no gateway ref yet)
      // F-1 FIX: Include escrow intent
  ...
  // CreateTransactionDTO.payment_method is now typed as 'visa' | 'fatora',
    ...
    try {
      ...
      for (const remaining of pendingItems) {
        const alreadyProcessed =
          succeededItems.some((s) => s.escrow_id === remaining.escrow_id) ||
  ...
  // so they never appear as stuck 'pending' in the user's wallet.
  if (failedItems.length > 0) {
    ...
    try {
      ...
      logger.error(
        'P1-NEW-001: Failed to cancel orphaned escrow entries — manual reconciliation required',
        {
          escrow_ids: failedEscrowIds,
    ...
    for (const gatewayResult of succeededItems) {
      ...
      const updatedEscrow = await client.query<EscrowLedger>(
        `UPDATE escrow_ledger
                 ...
                 RETURNING transaction_id, user_id, item_id, project_id,
      ...
      if (newFunded >= item.total_cost) {
        await client.query("UPDATE itemized_boq SET status = 'fully_funded' WHERE item_id = $1", [
          item.item_id,
        ]);
  ...
  if (failedItems.length > 0) {
    logger.warn('P1-NEW-001: Partial payment — some items failed', {
      user_id: userId,
...
// ─── Auto PO Generation (triggered from payment) ──────────────────────────
...
async function autoGeneratePO(
  ...
  if (boqItem.preferred_supplier_id) {
    ...
    if (!supplier) {
      logger.error('Pre-assigned supplier is no longer active/verified', {
        supplierId: boqItem.preferred_supplier_id,
        itemId: itemId,
      });
      return;
    }
