...
export async function exportProjectsPDF(res: Response, projectId?: string): Promise<void> {
  ...
  doc
    ...
    .font('Helvetica')
    .fillColor('#666666')
    .text(`OCDS-Compliant Report • Generated ${new Date().toISOString().slice(0, 10)}`, {
      align: 'center',
    });
  doc.moveDown(1);
  ...
  doc.fontSize(12).font('Helvetica-Bold').fillColor('#1E293B').text('Summary Statistics');
  doc.moveDown(0.5);
  ...
  // ── Project Table
  doc.fontSize(12).font('Helvetica-Bold').fillColor('#1E293B').text('Project Details');
  doc.moveDown(0.5);
  ...
  doc
    ...
    .fillColor('#94A3B8')
    .text(
      'This report is generated by Nammerha Platform in compliance with Open Contracting Data Standard (OCDS).',
      {
        align: 'center',
      },
    );
