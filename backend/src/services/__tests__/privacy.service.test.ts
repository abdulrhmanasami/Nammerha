...
describe('Privacy Service', () => {
      ...
      const result = applyPrivacyFilter(
        {
          user_id: 'u1',
          company_name: 'Co',
          commercial_license_number: 'CL-123',
          commercial_license_url: 'url',
          created_at: '',
        },
        settings,
        'project_member',
  ...
  describe('getDefaultSettingsForRole', () => {
    it('should return defaults for known roles', () => {
      const roles = ['contractor', 'engineer', 'supplier', 'tradesperson', 'homeowner', 'user'];
      for (const role of roles) {
        const settings = getDefaultSettingsForRole(role);
        expect(settings).not.toBeNull();
        if (!settings) {
          throw new Error('Expected non-null settings');
        }
  ...
  describe('getConfigurableFields', () => {
    ...
    it('should NOT include dangerous field names', () => {
      const allRoles = ['contractor', 'engineer', 'supplier', 'tradesperson', 'homeowner', 'user'];
