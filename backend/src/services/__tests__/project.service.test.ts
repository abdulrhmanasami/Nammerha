...
describe('Project Service', () => {
  ...
  describe('assignEngineer', () => {
    ...
    it('should throw when project not found', async () => {
      ...
      await expect(assignEngineer('nonexistent')).rejects.toThrow('not found');
    });
    ...
    it('should throw when project status is not draft', async () => {
      ...
      await expect(assignEngineer('OCDS-SYR-00001')).rejects.toThrow(
        "status is 'published', expected 'draft'",
      );
    });
    ...
    it('should throw when no verified engineer available', async () => {
      ...
      await expect(assignEngineer('OCDS-SYR-00001')).rejects.toThrow(
        'No verified engineers available',
      );
    });
    ...
    it('should require KYC + guild membership in engineer query', async () => {
      ...
      try {
        await assignEngineer('OCDS-SYR-00001');
      } catch {
        /* expected */
      }
  ...
  describe('addBOQItem', () => {
    it('should add BOQ item with oracle price and transition to assessed', async () => {
      ...
      expect(clientQuery).toHaveBeenCalledWith(expect.stringContaining("SET status = 'assessed'"), [
        'OCDS-SYR-00001',
      ]);
    });
    ...
    it('should throw when engineer is not assigned to project', async () => {
      ...
      clientQuery.mockResolvedValueOnce({
        rows: [
          { ...MOCK_PROJECT, status: 'pending_assessment', assigned_engineer_id: 'other-eng' },
        ],
      });

      await expect(
        addBOQItem('OCDS-SYR-00001', 'eng-1', {
          material_name: 'Cement',
          unit: 'bag',
          unit_price: 1500,
          required_quantity: 100,
    ...
    it('should throw when supplier is not KYC-verified', async () => {
      ...
      clientQuery.mockResolvedValueOnce({
        rows: [
          { user_id: 'sup-2', full_name: 'Unverified Co', kyc_verification_status: 'pending' },
        ],
      });
  ...
  describe('publishProject', () => {
    ...
    it('should throw when project has no BOQ items', async () => {
      ...
      await expect(publishProject('OCDS-SYR-00001', 'eng-1')).rejects.toThrow('no BOQ items');
    });
    ...
    it('should throw when status is not assessed', async () => {
      ...
      await expect(publishProject('OCDS-SYR-00001', 'eng-1')).rejects.toThrow(
        "status is 'draft', expected 'assessed'",
      );
    });
