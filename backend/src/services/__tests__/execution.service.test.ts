...
describe('Execution Service', () => {
    ...
    describe('updatePOStatus', () => {
        ...
        it('should throw for invalid status', async () => {
            await expect(updatePOStatus('po-1', 'hacked' as never, 'admin-1'))
                .rejects.toThrow('Invalid PO status');
        });

        it('should throw when PO not found', async () => {
            mockQuery.mockResolvedValueOnce({ rows: [] });

            await expect(updatePOStatus('nonexistent', 'shipped', 'admin-1'))
        ...
        it('should write audit trail entry after status update', async () => {
            mockQuery
                .mockResolvedValueOnce({ rows: [{ ...MOCK_PO, status: 'delivered', po_number: 'PO-SYR-00001' }] })
        ...
        it('should use CASE-based SQL (no dynamic column injection)', async () => {
            ...
            expect(sql).toContain("CASE WHEN $1 = 'acknowledged'     THEN NOW() ELSE acknowledged_at END");
    ...
    describe('submitSpatialProof — SSRF protection', () => {
        it('should reject proof with private IP image URL', async () => {
            ...
            await expect(submitSpatialProof('eng-1', {
                project_id: 'proj-1',
                item_id: 'item-1',
                gps_lat: 33.5138,
                gps_lng: 36.2765,
