...
describe('Homeowner Service', () => {
  ...
  describe('getMyProjects', () => {
    it('should return projects with engineer/contractor names and bid counts', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('WHERE p.homeowner_id = $1'), [
        'homeowner-1',
      ]);
    });
  });
  ...
  describe('getProjectBids', () => {
    it('should throw when homeowner does not own the project', async () => {
      ...
      await expect(getProjectBids('not-owner', 'project-1')).rejects.toThrow('not the owner');
    });
  ...
  describe('createServiceRequest', () => {
    ...
    it('should throw when INSERT returns no rows', async () => {
      ...
      await expect(
        createServiceRequest('homeowner-1', {
  ...
  describe('getMyApprovals', () => {
    ...
    it('should apply status filter when provided', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(expect.any(String), ['homeowner-1', 'pending']);
    });
  });
  ...
  describe('cancelServiceRequest', () => {
    ...
    it('should throw when request not found', async () => {
      ...
      await expect(cancelServiceRequest('homeowner-1', 'nonexistent')).rejects.toThrow(
    ...
    it('should throw when homeowner does not own the request', async () => {
      ...
      await expect(cancelServiceRequest('homeowner-1', 'sr-3')).rejects.toThrow('not the owner');
    });
    ...
    it('should prevent cancelling in_progress requests (TOCTOU protection)', async () => {
      ...
      await expect(cancelServiceRequest('homeowner-1', 'sr-4')).rejects.toThrow(
        'Can only cancel open or matched',
      );
    });
