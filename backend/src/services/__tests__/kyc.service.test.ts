...
describe('KYC Service', () => {
  ...
  describe('updateKycStatus', () => {
    ...
    it('should throw when user_id is empty', async () => {
      await expect(updateKycStatus('', 'verified', 'admin-1')).rejects.toThrow(
        'user_id is required',
      );
    });

    it('should throw when decision is invalid', async () => {
      await expect(updateKycStatus('u1', 'hacked' as never, 'admin-1')).rejects.toThrow(
        'decision must be',
      );
    });
    ...
    it('should throw when user not found or already processed', async () => {
      ...
      await expect(updateKycStatus('nonexistent', 'verified', 'admin-1')).rejects.toThrow(
        'not found or KYC status already processed',
      );
    });
    ...
    it('should only allow update from pending/submitted states', async () => {
      ...
      await updateKycStatus('u1', 'verified', 'admin-1').catch(() => {
        /* expected */
      });
