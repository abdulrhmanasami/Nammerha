...
// Covers: getMarketplaceProjects, getProjectBOQ, createDonation,
//         getDonorEscrowSummary, getDonorDonations, getVerifiedSuppliers
//
...
describe('Crowdfunding Service', () => {
    ...
    describe('createDonation', () => {
        it('should reject when BOQ item not found', async () => {
            ...
            await expect(createDonation('donor-1', {
        ...
        it('should cap donation at remaining need (prevent over-funding)', async () => {
            ...
            let callCount = 0;
            mockTransactionFn.mockImplementation(async (fn: (client: { query: typeof finalClientQuery }) => unknown) => {
                callCount++;
                if (callCount === 1) {
            ...
            mockTransactionFn.mockImplementation(async (fn: (client: { query: typeof vi.fn }) => unknown) => {
                txCount++;
                if (txCount === 1) {
            ...
            phase3Query.mockResolvedValueOnce({
                rows: [{
                    material_name: 'Cement', unit: 'bag', unit_price: 10000,
                    required_quantity: 5, preferred_supplier_id: 'sup-1',
                }],
            });
            phase3Query.mockResolvedValueOnce({
                rows: [{ user_id: 'sup-1', full_name: 'Supplier One', commercial_register_number: 'CR-123' }],
            });
            phase3Query.mockResolvedValueOnce({
        ...
        it('should cancel all pending escrow entries when gateway fails', async () => {
            ...
            mockTransactionFn.mockImplementation(async (fn: (client: { query: typeof phase1Query }) => unknown) => {
                txCall.push(fn);
                return fn({ query: phase1Query });
            });
    ...
    describe('getDonorEscrowSummary', () => {
        it('should return null when no summary found', async () => {
            ...
            const summary = await getDonorEscrowSummary('donor-no-history');
        ...
        it('should return escrow summary for existing donor', async () => {
            ...
            const summary = await getDonorEscrowSummary('donor-1');
    ...
    describe('getDonorDonations', () => {
        it('should enforce max limit of 50', async () => {
            ...
            await getDonorDonations('donor-1', 999);
        ...
        it('should enforce min limit of 1', async () => {
            ...
            await getDonorDonations('donor-1', -5);
