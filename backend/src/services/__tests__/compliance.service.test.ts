...
describe('Compliance Service', () => {
    ...
    describe('screenUserAgainstSDN', () => {
        it('should return "clear" when no SDN match above threshold', async () => {
            ...
            mockQuery
                .mockResolvedValueOnce({
                    rows: [{ user_id: 'user-1', full_name: 'Ahmad Hammoud', role: 'donor' }],
                    rowCount: 1,
                })
                // Mock 2: No SDN match
        ...
        it('should throw when user is not found', async () => {
            ...
            await expect(screenUserAgainstSDN('nonexistent'))
    ...
    describe('reviewScreeningResult', () => {
        it('should clear false_positive and reactivate auto-blocked user', async () => {
            ...
            const result = await reviewScreeningResult('sr-4', 'admin-1', 'false_positive', 'Name mismatch');
        ...
        it('should throw when screening result not found', async () => {
            ...
            await expect(reviewScreeningResult('nonexistent', 'admin-1', 'false_positive'))
    ...
    describe('importSDNList', () => {
        ...
        it('should handle entries with missing optional fields', async () => {
            ...
            expect(mockQuery).toHaveBeenCalledWith(
                expect.any(String),
                expect.arrayContaining(['Minimal Entry', 'individual', null, null, null, 'OFAC_SDN', null, null])
            );
        });
    });
    ...
    describe('checkDualUse', () => {
        ...
        it('should flag dual-use material and update BOQ item', async () => {
            // Match found
            mockQuery.mockResolvedValueOnce({
                rows: [{ material_id: 'cm-1', material_name: 'Ammonium Nitrate', regulation: 'EAR', match_score: '0.85' }],
                rowCount: 1,
            });
            // UPDATE itemized_boq
            ...
            expect(mockQuery).toHaveBeenCalledWith(
                expect.stringContaining('UPDATE itemized_boq'),
                ['EAR', 'item-2']
            );
        });
        ...
        it('should NOT flag when match score is exactly 0.3 or below', async () => {
            mockQuery.mockResolvedValueOnce({
                rows: [{ material_id: 'cm-2', material_name: 'Sodium Chloride', regulation: 'EAR', match_score: '0.25' }],
                rowCount: 1,
            });
    ...
    describe('getDualUseItems', () => {
        it('should return flagged items with project title', async () => {
            ...
            expect(mockQuery).toHaveBeenCalledWith(
                expect.stringContaining('WHERE b.is_dual_use = true')
            );
        });
    });
});
