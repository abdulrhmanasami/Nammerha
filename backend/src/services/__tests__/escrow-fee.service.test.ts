 ...
 * Commercial = only the homeowner funds the project (no external donors).
 */
function isCommercialProject(
    homeownerId: string,
    donorIds: string[],
): boolean {
    // If no external donors (all funding from homeowner), it's commercial
    const externalDonors = donorIds.filter(id => id !== homeownerId);
    return externalDonors.length === 0;
}
...
describe('isCommercialProject (Fee Exemption)', () => {
    ...
    it('returns true when no donors at all', () => {
        expect(isCommercialProject('user-123', [])).toBe(true);
    });

    it('returns false when external donors contribute', () => {
        expect(isCommercialProject('user-123', ['donor-1', 'donor-2'])).toBe(false);
    });

    it('returns false when mix of homeowner and external donors', () => {
        expect(isCommercialProject('user-123', ['user-123', 'donor-1'])).toBe(false);
    });
...
describe('End-to-End Fee Calculation Flow', () => {
    ...
    it('humanitarian project: $10,000 release → $0 fee (exempt)', () => {
        const isCommercial = isCommercialProject('owner', ['donor-1', 'donor-2']);
