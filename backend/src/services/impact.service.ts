...
const TEMPLATES: Record<ImpactEventType, MessageTemplate> = {
    donation_received: {
        ...
        body_en: 'Your generous contribution of ${{amount}} has been securely locked in escrow for "{{project_title}}". The funds will be released to verified suppliers as construction materials are delivered and verified by GPS.',
        body_ar: 'تم تسجيل تبرعك السخي بقيمة ${{amount}} بأمان في حساب الضمان لمشروع "{{project_title}}". سيتم إفراج الأموال للموردين المعتمدين عند تسليم مواد البناء والتحقق منها بنظام GPS.',
    },
    contractor_assigned: {
        title_en: 'Contractor Assigned to Your Project 👷',
        title_ar: 'تم تعيين مقاول لمشروعك 👷',
        body_en: 'A licensed contractor has been assigned to "{{project_title}}". They will coordinate with the assigned engineer to begin reconstruction work. You can track progress from your Impact Dashboard.',
        body_ar: 'تم تعيين مقاول مرخّص لمشروع "{{project_title}}". سيتنسق مع المهندس المعيّن لبدء أعمال إعادة الإعمار. يمكنك متابعة التقدم من لوحة الأثر.',
    },
    construction_started: {
        title_en: 'Construction Has Begun! 🚧',
        title_ar: 'بدأ البناء! 🚧',
        body_en: 'Exciting news! Work has officially started on "{{project_title}}". Our on-site engineer will verify each phase with GPS-stamped photos so you can see the real impact of your contribution.',
        body_ar: 'أخبار رائعة! بدأ العمل رسمياً في مشروع "{{project_title}}". سيقوم المهندس الميداني بالتحقق من كل مرحلة بصور مختومة بالموقع الجغرافي لتشاهد الأثر الحقيقي لمساهمتك.',
    },
    milestone_completed: {
        title_en: 'Milestone Reached: {{milestone}} 📊',
        title_ar: 'تم إنجاز مرحلة: {{milestone}} 📊',
        body_en: '"{{project_title}}" has reached {{progress}}% completion! The {{milestone}} phase has been verified by the assigned engineer. Your donation is making a tangible difference.',
        body_ar: 'وصل مشروع "{{project_title}}" إلى {{progress}}% إتمام! تم التحقق من مرحلة {{milestone}} بواسطة المهندس المعيّن. تبرعك يُحدث فرقاً ملموساً.',
    },
    photo_proof_added: {
        title_en: 'New Photo from Your Project 📸',
        title_ar: 'صورة جديدة من مشروعك 📸',
        body_en: 'A GPS-verified photo has been uploaded for "{{project_title}}". Visit your Proof Gallery to see the latest construction progress — verified at coordinates ({{lat}}, {{lng}}).',
        body_ar: 'تم رفع صورة موثقة بالموقع الجغرافي لمشروع "{{project_title}}". زُر معرض الإثباتات لمشاهدة آخر تقدم في البناء — موثقة عند الإحداثيات ({{lat}}, {{lng}}).',
    },
    escrow_released: {
        title_en: 'Funds Released to Supplier 💸',
        title_ar: 'تم إفراج الأموال للمورّد 💸',
        body_en: '${{amount}} from your donation has been released to the verified supplier for "{{material_name}}" on project "{{project_title}}". The material delivery was confirmed with GPS proof.',
        body_ar: 'تم إفراج ${{amount}} من تبرعك للمورّد المعتمد مقابل "{{material_name}}" في مشروع "{{project_title}}". تم تأكيد تسليم المواد بإثبات GPS.',
    },
    project_completed: {
        title_en: '🎉 Project Completed! Your Impact is Real',
        title_ar: '🎉 اكتمل المشروع! أثرك حقيقي',
        body_en: 'Wonderful news! "{{project_title}}" has been officially completed. Your contribution of ${{amount}} helped rebuild a home and restore dignity to a family. Thank you for making this possible.',
        body_ar: 'أخبار رائعة! اكتمل مشروع "{{project_title}}" رسمياً. ساعدت مساهمتك بقيمة ${{amount}} في إعادة بناء منزل واستعادة الكرامة لعائلة. شكراً لجعلك هذا ممكناً.',
    },
};
 ...
 * @param donorId    — UUID of the donor to notify
...
export async function generateImpactMessage(
    ...
    logger.info('Impact message generated', { eventType, donorId, projectId });
...
export async function notifyAllProjectDonors(
    ...
    for (const donor of donors.rows) {
        try {
            await generateImpactMessage(eventType, donor.donor_id, projectId, {
                ...data,
                amount: parseInt(donor.total_donated, 10),
            });
            count++;
        } catch (error) {
            logger.error('Failed to generate impact message for donor', {
                eventType, donorId: donor.donor_id, projectId, error,
            });
        }
    }
 ...
 */
export async function getUnreadCount(donorId: string): Promise<number> {
    const result = await query<{ count: string }>(
        `SELECT COUNT(*) AS count FROM impact_messages
         WHERE donor_id = $1 AND read_at IS NULL`,
        [donorId],
    );
...
export async function markAsRead(
    ...
    const result = await query(
        `UPDATE impact_messages
         SET read_at = NOW()
         WHERE message_id = $1 AND donor_id = $2 AND read_at IS NULL`,
        [messageId, donorId],
    );
 ...
 */
export async function markAllRead(donorId: string): Promise<number> {
    const result = await query(
        `UPDATE impact_messages
         SET read_at = NOW()
         WHERE donor_id = $1 AND read_at IS NULL`,
