 ...
 */
export async function getMyProjects(homeownerId: string): Promise<HomeownerProject[]> {
 ...
 */
export async function getMyStats(homeownerId: string): Promise<HomeownerStats> {
 ...
 */
export async function getMyEscrowSummary(homeownerId: string): Promise<EscrowSummary> {
  const result = await query<{
    deposited: string;
    released: string;
...
export function cancelServiceRequest(
  ...
  return financialTransaction(async (client) => {
    ...
    await client.query(`UPDATE service_requests SET status = 'cancelled' WHERE request_id = $1`, [
      requestId,
    ]);
