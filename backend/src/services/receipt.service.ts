 ...
 *
 * @param donorId  Authenticated donor's user_id (ownership check)
...
export async function generateReceipt(
    ...
    const cacheKey = `${donorId}:${escrowId}`;
    ...
    const result = await query<ReceiptData>(
        `SELECT
         ...
         WHERE el.transaction_id = $1 AND el.donor_id = $2`,
        [escrowId, donorId],
    );
...
function buildPdfBuffer(data: ReceiptData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
        ...
        if (data.donation_intent && data.donation_intent !== 'general') {
            addField('Intent / النية', data.donation_intent === 'zakat' ? 'زكاة / Zakat' : 'صدقة / Sadaqah');
        }
