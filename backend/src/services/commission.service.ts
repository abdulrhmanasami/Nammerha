...
export async function recordCommissionInTransaction(
    ...
    // commercial (homeowner-funded) projects. Donor-funded humanitarian
...
export async function getSupplierCommissions(
    ...
    const [entriesRes, countRes] = await Promise.all([
        ...
        query<{ count: string }>(
            `SELECT COUNT(*) FROM commission_ledger WHERE supplier_id = $1`,
            [supplierId],
        ),
    ]);
...
export async function getAllCommissions(
    ...
    const [entriesRes, countRes] = await Promise.all([
        ...
        query<{ count: string }>(
            `SELECT COUNT(*) FROM commission_ledger cl ${whereClause}`,
            params,
        ),
    ]);
