...
export async function submitSpatialProof(
  ...
  return financialTransaction(async (client) => {
    ...
    if (!project) {
      throw new Error(`Project ${dto.project_id} not found`);
    }
    ...
    try {
      ...
      if (dto.client_hash && dto.client_hash.toLowerCase() !== imageHash.toLowerCase()) {
        ...
        throw new Error(
          'Spatial proof integrity compromise detected: Client hash does not match server hash. Refusing proof.',
        );
      }
    } catch (hashErr) {
 ...
 */
export async function getProjectPurchaseOrders(projectId: string): Promise<PurchaseOrder[]> {
