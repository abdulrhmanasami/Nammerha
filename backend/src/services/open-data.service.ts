        ...
        ? [
            {
              ...
              milestones: milestoneRes.rows.map((m: Record<string, unknown>) => ({
                ...
                status: (m.completed_at as Date | null) ? 'met' : 'scheduled',
...
export async function getPlatformStats(): Promise<PlatformStats> {
  const res = await pool.query(`
        SELECT
            ...
            (SELECT COUNT(DISTINCT ur.user_id) FROM user_roles ur JOIN roles r ON r.role_id = ur.role_id AND r.role_name = 'user' JOIN users u ON u.user_id = ur.user_id AND u.is_active = true WHERE ur.status = 'active') AS total_users,
...
export function getOCDSExtensionSchema(): Record<string, unknown> {
  return {
    ...
    description:
      'OCDS extension adding GPS-stamped spatial proofs and 360° reality captures to release packages for anti-fraud verification in post-conflict reconstruction.',
              ...
              realityCaptures: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    type: {
                      type: 'string',
                      enum: ['photo_360', 'video_360', 'point_cloud', 'photo_standard'],
                    },
