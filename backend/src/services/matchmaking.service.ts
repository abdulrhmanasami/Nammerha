...
const weightSum =
  SCORE_WEIGHTS.completed_projects +
  SCORE_WEIGHTS.response_speed +
  SCORE_WEIGHTS.bid_win_rate +
  SCORE_WEIGHTS.license_status;
...
export function calculateScoringFactors(eng: EngineerMetrics): ScoringFactors {
  ...
  if (eng.avg_response_hours !== null) {
    responseFactor = Math.max(
      0,
      Math.min(100, 100 - ((Number(eng.avg_response_hours) - 24) / (168 - 24)) * 100),
    );
  }
  ...
  const compositeScore =
    Math.round(
      (SCORE_WEIGHTS.completed_projects * projectsFactor +
        ...
        SCORE_WEIGHTS.license_status * licenseFactor) *
        100,
    ) / 100;
...
export async function recalculateScore(engineerId: string): Promise<number> {
  ...
  try {
    ...
    // Persist
    await client.query(`UPDATE users SET dynamic_score = $1 WHERE user_id = $2`, [
      compositeScore,
      engineerId,
    ]);
 ...
 */
export async function matchProjectToEngineers(projectId: string): Promise<EngineerScore[]> {
...
async function enrichWithRoadDistances(
  ...
  const hasValidCoords =
    targets.every((t) => t.lat !== 0 && t.lng !== 0 && isFinite(t.lat) && isFinite(t.lng)) &&
    isFinite(projectLat) &&
    isFinite(projectLng) &&
    projectLat !== 0 &&
    projectLng !== 0;
  ...
  try {
    matrix = await getDistanceMatrix({ lat: projectLat, lng: projectLng }, targets, 'auto');
  } catch (error) {
...
export async function submitBid(
  ...
  return transaction(async (client) => {
    ...
    const scoreRes = await client.query(`SELECT dynamic_score FROM users WHERE user_id = $1`, [
      engineerId,
    ]);
    ...
    const { rows } = await client.query(
      `INSERT INTO contractor_bids
                (engineer_id, contractor_id, project_id, proposed_cost, currency, estimated_days,
                 cover_letter, methodology, engineer_score_snapshot)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
            RETURNING bid_id, engineer_id, contractor_id, project_id,
                      proposed_cost, currency, estimated_days, cover_letter, methodology,
