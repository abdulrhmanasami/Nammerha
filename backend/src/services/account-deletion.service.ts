...
export async function checkDeletionBlockers(userId: string): Promise<DeletionBlocker[]> {
    ...
    const escrowResult = await pool.query<{ count: string; total_locked: string }>(
        `SELECT COUNT(*) AS count, COALESCE(SUM(amount_locked), 0) AS total_locked
         FROM escrow_ledger
         WHERE donor_id = $1 AND payment_status = 'locked'`,
        [userId],
    );
    ...
    if (userRole === 'admin' || userRole === 'auditor') {
        blockers.push({
            type: 'admin_role',
            message: 'Admin and auditor accounts require manual deletion by a super-administrator. Please contact support.',
            message_ar: 'حسابات المدراء والمدققين تتطلب حذفاً يدوياً من قبل مدير أعلى. يرجى التواصل مع الدعم.',
        });
    }
...
export async function getDeletionStatus(userId: string): Promise<DeletionStatus> {
    const result = await pool.query<{
        ...
        `SELECT deleted_at, deletion_scheduled_at FROM users WHERE user_id = $1`,
        [userId],
    );
...
export async function executePermanentDeletion(userId: string): Promise<PurgeResult> {
    ...
    try {
        ...
        // 2a. escrow_ledger — donor_id stays for aggregate integrity,
        ...
        if ((escrowUpdate.rowCount ?? 0) > 0) result.anonymized_tables.push('escrow_ledger.released_by');
        ...
        if ((projEngineer.rowCount ?? 0) > 0) result.anonymized_tables.push('projects.assigned_engineer_id');
        ...
        await client.query(`UPDATE itemized_boq SET verified_by = NULL WHERE verified_by = $1`, [userId]);
        result.anonymized_tables.push('itemized_boq.created_by', 'itemized_boq.verified_by');

        // 2f. epa_adjustments — nullify approver
        await client.query(`UPDATE epa_adjustments SET approved_by = NULL WHERE approved_by = $1`, [userId]);
        result.anonymized_tables.push('epa_adjustments.approved_by');

        // 2g. compliance_records — nullify reviewer
        await client.query(`UPDATE compliance_records SET reviewed_by = NULL WHERE reviewed_by = $1`, [userId]);
        result.anonymized_tables.push('compliance_records.reviewed_by');

        // 2h. reality capture annotations — nullify author
        await client.query(
            `UPDATE capture_annotations SET author_id = NULL WHERE author_id = $1`,
            [userId],
        ...
        await client.query(
            `UPDATE engineer_assignments SET engineer_id = NULL WHERE engineer_id = $1`,
            [userId],
        ).catch(() => { /* Table may not exist */ });

        // 2k. payment_transactions — nullify donor reference
        await client.query(
            `UPDATE payment_transactions SET donor_id = NULL WHERE donor_id = $1`,
            [userId],
        ).catch(() => { /* Table may not exist */ });
        result.anonymized_tables.push('payment_transactions.donor_id');

        // 2l. review system — nullify reviewer references
        await client.query(
            `UPDATE review_reports SET reviewed_by = NULL WHERE reviewed_by = $1`,
            [userId],
        ).catch(() => { /* Table may not exist */ });
        ...
        const mfaDel = await client.query(`DELETE FROM user_recovery_codes WHERE user_id = $1`, [userId]).catch(() => null);
        if (mfaDel && (mfaDel.rowCount ?? 0) > 0) result.hard_deleted_tables.push('user_recovery_codes');

        const mfaSecDel = await client.query(`DELETE FROM user_mfa_secrets WHERE user_id = $1`, [userId]).catch(() => null);
        if (mfaSecDel && (mfaSecDel.rowCount ?? 0) > 0) result.hard_deleted_tables.push('user_mfa_secrets');

        // Social auth accounts
        const socialDel = await client.query(`DELETE FROM social_auth_accounts WHERE user_id = $1`, [userId]).catch(() => null);
        if (socialDel && (socialDel.rowCount ?? 0) > 0) result.hard_deleted_tables.push('social_auth_accounts');
        ...
        const profileTables = [
            'homeowner_profiles', 'engineer_profiles', 'contractor_profiles',
            'supplier_profiles', 'tradesperson_profiles', 'donor_profiles',
        ];
        for (const table of profileTables) {
            const delResult = await client.query(`DELETE FROM ${table} WHERE user_id = $1`, [userId]).catch(() => null);
        ...
        await client.query(
            `DELETE FROM compliance_records WHERE user_id = $1`,
            [userId],
        );
        result.hard_deleted_tables.push('compliance_records');

        // Supplier catalog
        await client.query(`DELETE FROM supplier_catalog WHERE supplier_id = $1`, [userId]).catch(() => {});
        ...
        await client.query(`DELETE FROM review_responses WHERE responder_id = $1`, [userId]).catch(() => {});
        await client.query(`DELETE FROM review_reports WHERE reporter_id = $1`, [userId]).catch(() => {});
        ...
        // Impact messages
        await client.query(`DELETE FROM impact_messages WHERE donor_id = $1`, [userId]).catch(() => {});
        result.hard_deleted_tables.push('impact_messages');

        // Subscriptions
        await client.query(`DELETE FROM user_subscriptions WHERE user_id = $1`, [userId]).catch(() => {});
        result.hard_deleted_tables.push('user_subscriptions');

        // Matchmaking applications
        await client.query(`DELETE FROM matchmaking_applications WHERE engineer_id = $1`, [userId]).catch(() => {});
