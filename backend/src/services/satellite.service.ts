...
export async function registerImagery(
    ...
    if (!VALID_PROVIDERS.includes(dto.provider as SatelliteProvider)) {
        throw new Error(
            `Invalid provider '${dto.provider}'. Allowed: ${VALID_PROVIDERS.join(', ')}`
        );
    }
    ...
    const { rows } = await pool.query(
        ...
        [
            dto.project_id,
            dto.bbox_wkt,
            dto.center_lng, dto.center_lat,     // PostGIS: lon, lat order
            dto.captured_at,
            dto.provider, dto.resolution_cm, dto.sensor_name ?? null, dto.band_count ?? null,
            dto.image_url, dto.thumbnail_url ?? null, JSON.stringify(dto.stac_metadata ?? {}),
            dto.cloud_cover_pct ?? null, dto.quality_score ?? null, adminId, dto.notes ?? null,
        ],
    );
...
export async function deleteImagery(imageryId: string): Promise<boolean> {
    const { rowCount } = await pool.query(
        `DELETE FROM satellite_imagery WHERE imagery_id = $1`,
        [imageryId],
    );
    return (rowCount ?? 0) > 0;
}
