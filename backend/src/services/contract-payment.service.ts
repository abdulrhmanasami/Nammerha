 ...
 */
export async function getMilestones(contractId: string, userId: string): Promise<MilestoneRow[]> {
  // Verify access
  ...
  return financialTransaction(async (client: PoolClient) => {
    ...
    const contractResult = await client.query<ContractRow>(
      `SELECT contract_id, project_id, homeowner_id, provider_id, provider_type,
                    total_agreed_amount, bid_id, notes, status, currency, created_at, updated_at
...
export async function confirmPayment(
  ...
  return financialTransaction(async (client: PoolClient) => {
    ...
    const paymentResult = await client.query<
      PaymentRow & { homeowner_id: string; provider_id: string }
    >(
