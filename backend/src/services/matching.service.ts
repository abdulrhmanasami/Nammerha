...
export async function createProgram(
  ...
  if (!program) {
    throw new Error('Failed to create matching program');
  }
 ...
 */
export async function findMatchingPrograms(projectId: string): Promise<MatchingProgram[]> {
  // Get project details for filter matching
  const projectResult = await pool.query<{
    damage_type: string;
    address_text: string | null;
  }>('SELECT damage_type, address_text FROM projects WHERE project_id = $1', [projectId]);
    ...
    try {
      const pledge = await transaction(async (client: PoolClient) => {
        ...
        const escrowResult = await client.query<{ transaction_id: string }>(
          ...
          [locked.sponsor_id, itemId, projectId, matchAmount, `match-${locked.program_id}`],
        );

        const matchedEscrowId = escrowResult.rows[0]?.transaction_id;
        if (!matchedEscrowId) {
          throw new Error('Failed to create matched escrow');
        }
        ...
        if (!p) {
          throw new Error('Failed to create matching pledge');
        }
