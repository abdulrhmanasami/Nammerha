...
//   2. Financial mutations (payment initiation, donations) MUST NOT skip.
...
export function requireIdempotencyKey(
    ...
    if (!rawKey) {
        res.status(400).json({
            success: false,
            error: 'Missing required Idempotency-Key header. Each financial request must include a unique idempotency key to prevent duplicate processing.',
    ...
    if (/[\x00-\x1f<>"'`;\\]/.test(key)) {
        res.status(400).json({
            success: false,
            error: 'Idempotency-Key contains invalid characters. Use alphanumeric characters, hyphens, underscores, dots, or colons.',
