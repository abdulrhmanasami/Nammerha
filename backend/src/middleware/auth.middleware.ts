...
if (!AUTH0_ENABLED && !JWT_SECRET) {
  const msg =
    '[AUTH FATAL] Neither AUTH0_DOMAIN/AUTH0_AUDIENCE nor JWT_SECRET is configured. ' +
...
export function generateToken(
  ...
  // P0-AUTH-001 FIX: Support configurable expiry for Remember Me (30d).
  // Priority: customExpiresIn > JWT_EXPIRY env > '24h' default.
  const expiresIn = customExpiresIn ?? process.env['JWT_EXPIRY'] ?? '24h';
  return jwt.sign({ sub: userId, role, roles: roles ?? [role] }, JWT_SECRET, {
    expiresIn,
    algorithm: 'HS256',
  } as jwt.SignOptions);
}
