...
const PROFILE_TABLE_MAP: Record<string, string> = {
  // FORENSIC-C2.5: 'user' removed — payment system suspended indefinitely (2026-05-12).
  // user_profiles table preserved in DB for backward compat, but no new activations.
...
router.post('/activate', async (req: Request, res: Response) => {
  try {
    ...
    const upsertResult = await query<{ status: string; was_existing: boolean }>(
      ...
      [req.authUser.user_id, roleData.role_id, initialStatus, req.authUser.roles.length === 0],
    );
