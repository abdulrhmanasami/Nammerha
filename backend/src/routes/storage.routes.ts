...
async function hasProjectAccess(
  ...
  const result = await query<{ cnt: string }>(
    `SELECT COUNT(*) AS cnt FROM projects
         WHERE project_id = $1
           AND (homeowner_id = $2 OR assigned_engineer_id = $2)`,
    [projectId, userId],
  );
...
async function verifyProjectAccessParam(
  ...
  if (!allowed) {
    logger.warn('Storage: IDOR blocked — project access denied', {
...
async function verifyProjectAccessFromKey(
  ...
  if (!allowed) {
    logger.warn('Storage: IDOR blocked — file access denied', {
      userId: getAuthUser(req).user_id,
      fileKey,
    });
    res.status(403).json({
...
router.post('/upload-url', async (req: Request, res: Response) => {
  try {
    const { project_id, category, filename, content_type, file_size_bytes } =
      storageUploadUrlSchema.parse(req.body);
    ...
    const allowed =
      isPending ||
      (await hasProjectAccess(getAuthUser(req).user_id, getAuthUser(req).role, project_id));

    if (!allowed) {
      logger.warn('Storage: IDOR blocked — upload denied', {
        userId: getAuthUser(req).user_id,
