...
import { validateUploadRequest, generateFileKey, type UploadUrlRequest } from '../../services/storage.service';
...
describe('validateUploadRequest() — Upload Validation (Pure Unit)', () => {
    ...
    it('should throw for invalid category', () => {
        expect(() => validateUploadRequest({ ...VALID_UPLOAD, category: 'malware' as never }))
            .toThrow('Invalid upload category');
    });

    it('should throw for disallowed MIME type', () => {
        expect(() => validateUploadRequest({ ...VALID_UPLOAD, content_type: 'application/exe' }))
            .toThrow("not allowed for category 'proof'");
    });

    it('should throw for oversized file', () => {
        expect(() => validateUploadRequest({ ...VALID_UPLOAD, file_size_bytes: 500_000_000 }))
            .toThrow('File size exceeds maximum');
    });

    it('should throw for filename exceeding length limit', () => {
        const longName = 'a'.repeat(201) + '.jpg';
        expect(() => validateUploadRequest({ ...VALID_UPLOAD, filename: longName }))
            .toThrow('Invalid filename');
    });

    it('should throw for filename with invalid characters', () => {
        expect(() => validateUploadRequest({ ...VALID_UPLOAD, filename: 'file<script>.jpg' }))
            .toThrow('invalid characters');
    });

    it('should throw for empty filename', () => {
        expect(() => validateUploadRequest({ ...VALID_UPLOAD, filename: '' }))
            .toThrow('Invalid filename');
    });

    it('should throw for invalid project_id format (non-UUID)', () => {
        expect(() => validateUploadRequest({ ...VALID_UPLOAD, project_id: 'not-a-uuid' }))
            .toThrow('Invalid project_id format');
    });

    it('should accept valid UUID project_id', () => {
        expect(() => validateUploadRequest({ ...VALID_UPLOAD, project_id: VALID_UUID }))
            .not.toThrow();
    });
