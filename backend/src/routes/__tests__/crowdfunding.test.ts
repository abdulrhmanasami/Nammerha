...
//   3. createPaymentIntent — 3-phase escrow lock (validate → gateway → finalize)
//   4. createPaymentIntent — BigInt arithmetic safety
//   5. createPaymentIntent — over-funding prevention
//   6. getUserEscrowSummary — summary aggregation
//   7. getUserPayments — paginated history with joins
...
describe('Crowdfunding Service', () => {
  ...
  describe('getMarketplaceProjects()', () => {
    ...
    it('should filter by damage_type when provided', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('damage_type = $1'), [
        'structural',
        25,
        0,
      ]);
  ...
  // ─── createPaymentIntent (3-phase escrow lock) ───────────────────────────
  describe('createPaymentIntent()', () => {
    it('should execute 3-phase payment flow successfully', async () => {
      ...
      const result = await createPaymentIntent('user-001', {
    ...
    it('should throw when BOQ item does not exist', async () => {
      const mockClient = {
        query: vi.fn().mockResolvedValueOnce({ rows: [], rowCount: 0 }), // BOQ not found
      };
      mockTransaction.mockImplementationOnce(async (fn: (client: unknown) => Promise<unknown>) =>
        fn(mockClient),
      );
    ...
    it('should cap payment at remaining need to prevent over-funding', async () => {
      // Item needs $1000 total, already funded $800 → remaining = $200
      // user tries to donate $500 → should be capped at $200
      const mockClient1 = {
        query: vi
          .fn()
          .mockResolvedValueOnce({
      ...
      const mockClient3 = {
        query: vi
          ...
          .mockResolvedValueOnce({
            rows: [
              { transaction_id: 'escrow-002', amount_locked: 20000, payment_status: 'locked' },
            ],
            rowCount: 1,
          })
          .mockResolvedValueOnce({
    ...
    it('should cancel orphaned escrow entries when gateway fails mid-batch (P1-NEW-001)', async () => {
      ...
      const mockClient3 = {
        query: vi
          ...
          .mockResolvedValueOnce({
            rows: [
              { transaction_id: 'escrow-001', amount_locked: 100000, payment_status: 'locked' },
            ],
            rowCount: 1,
          })
          .mockResolvedValueOnce({
            rows: [{ funded_amount: 100000 }],
            rowCount: 1,
          })
          ...
          .mockResolvedValueOnce({
            rows: [
              { user_id: 'sup-001', full_name: 'Supplier', commercial_register_number: 'CR-1' },
            ],
            rowCount: 1,
          })
          .mockResolvedValueOnce({ rows: [], rowCount: 1 }),
    ...
    it('should process multi-item payment successfully', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        mockClient3.query
          .mockResolvedValueOnce({
            rows: [
              { transaction_id: `escrow-${i}`, amount_locked: 50000, payment_status: 'locked' },
            ],
            rowCount: 1,
          })
          .mockResolvedValueOnce({
            rows: [{ funded_amount: 50000 }],
            rowCount: 1,
          })
  ...
  describe('getUserEscrowSummary()', () => {
    it('should return summary from the view', async () => {
      mockQuery.mockResolvedValueOnce({
        rows: [{ user_id: 'user-001', total_locked: 1500000, total_released: 500000 }],
        rowCount: 1,
      });

      const result = await getUserEscrowSummary('user-001');

      expect(result).toBeTruthy();
      expect((result as Record<string, unknown>).total_locked).toBe(1500000);
      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('vw_user_escrow_summary'), [
        'user-001',
      ]);
    });

    it('should return null when user has no payments', async () => {
      mockQuery.mockResolvedValueOnce({ rows: [], rowCount: 0 });

      const result = await getUserEscrowSummary('user-nonexistent');
  ...
  describe('getUserPayments()', () => {
      ...
      const mockpayments = [
        {
          ...
          amount_locked: 500000,
          material_name: 'Cement',
          project_title: 'Aleppo School',
        },
      ];
      mockQuery.mockResolvedValueOnce({ rows: mockpayments, rowCount: 1 });

      const result = await getUserPayments('user-001', 10, 0);
    ...
    it('should use default pagination (limit=50, offset=0)', async () => {
      ...
      await getUserPayments('user-001');

      expect(mockQuery).toHaveBeenCalledWith(expect.anything(), ['user-001', 50, 0]);
    });
  });
  ...
  describe('getVerifiedSuppliers()', () => {
    it('should return only active, verified suppliers', async () => {
      const mockSuppliers = [
        {
          user_id: 's1',
          full_name: 'Damascus Materials Co.',
