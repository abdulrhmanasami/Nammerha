...
//   6. RBAC verification (admin/auditor ✓, user ✗)
// ============================================================================
...
describe('EPA Oracle Routes (HTTP Integration)', () => {
  ...
  describe('RBAC Guards', () => {
    it('should reject unauthenticated requests', async () => {
      mockAuthUser = null;
      const res = await request(app).get('/api/oracle/prices').expect(401);

      expect(res.body.error).toContain('Authentication required');
    });

    it('should reject user role from accessing oracle prices', async () => {
      mockAuthUser = {
      ...
      const res = await request(app).get('/api/oracle/prices').expect(403);
    ...
    it('should allow engineer to read oracle prices', async () => {
      ...
      const res = await request(app).get('/api/oracle/prices').expect(200);
    ...
    it('should reject user from EPA alerts', async () => {
      mockAuthUser = {
        user_id: 'homeowner-001',
        role: 'homeowner',
        roles: ['homeowner'],
  ...
  describe('GET /api/oracle/prices', () => {
    ...
    it('should filter by material_code query parameter', async () => {
      ...
      await request(app).get('/api/oracle/prices?material_code=STL-001').expect(200);

      expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('WHERE material_code = $1'), [
        'STL-001',
      ]);
    });
  });
  ...
  describe('GET /api/oracle/epa/history/:projectId', () => {
    it('should return EPA history for a project', async () => {
      ...
      const res = await request(app).get('/api/oracle/epa/history/proj-001').expect(200);
  ...
  describe('GET /api/oracle/epa/alerts', () => {
    it('should return threshold alerts', async () => {
      mockQuery.mockResolvedValueOnce({
        rows: [{ project_id: 'proj-001', material_name: 'Steel', price_change_pct: 8.5 }],
        rowCount: 1,
      });

      const res = await request(app).get('/api/oracle/epa/alerts').expect(200);
