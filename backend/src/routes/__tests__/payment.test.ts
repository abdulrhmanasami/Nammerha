...
const WEBHOOK_SECRET =
  process.env['PAYMENT_WEBHOOK_SECRET'] ?? 'test-webhook-secret-for-vitest-never-use-in-production';
...
describe('Payment Routes (HTTP Integration)', () => {
  ...
  describe('GET /status/:ref — IDOR Protection (MED-AUD-003)', () => {
    it('should return 404 for non-existent payment reference', async () => {
      ...
      const res = await request(app).get('/status/NM-PAY-NONEXISTENT').expect(404);
    ...
    it('should return payment data to the payment owner', async () => {
      ...
      const res = await request(app).get('/status/NM-PAY-OWNED').expect(200);
    ...
    it("should return 403 when non-owner queries someone else's payment", async () => {
      ...
      const res = await request(app).get('/status/NM-PAY-NOTMINE').expect(403);
    ...
    it('should allow admin to view any payment', async () => {
      ...
      const res = await request(app).get('/status/NM-PAY-OTHERS').expect(200);
    ...
    it('should allow auditor to view any payment', async () => {
      ...
      const res = await request(app).get('/status/NM-PAY-ANY').expect(200);
  ...
  describe('Webhook Signature Verification (Unit)', () => {
    ...
    it('should produce different signatures for different payloads', () => {
      expect(createSignature('payload-A', TEST_SECRET)).not.toBe(
        createSignature('payload-B', TEST_SECRET),
      );
    });
