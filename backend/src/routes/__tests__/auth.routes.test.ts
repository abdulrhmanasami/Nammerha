...
describe('Auth Routes (HTTP Integration)', () => {
  ...
  describe('POST /api/auth/register', () => {
    ...
    it('should return 400 when required fields are missing', async () => {
      ...
      expect(res.body.error).toContain('Invalid registration data');
    });

    it('should return 400 for partial fields (missing password)', async () => {
      const res = await request(app)
        .post('/api/auth/register')
        .send({ email: 'a@b.com', full_name: 'Test', role: 'homeowner' })
        .expect(400);
    ...
    it('should return 400 for invalid email format', async () => {
      ...
      expect(res.body.success).toBe(false);
      expect(res.body.error).toBe('Invalid registration data. Please check all fields.');
    });
    ...
    it('should return 200 with generic message when email already exists (anti-enumeration)', async () => {
      ...
      const res = await request(app).post('/api/auth/register').send(VALID_BODY).expect(200);
