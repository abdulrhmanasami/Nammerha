...
describe('Matchmaking Routes (HTTP Integration)', () => {
  ...
  describe('Authentication', () => {
    it('should reject unauthenticated requests', async () => {
      mockAuthUser = null;
      const res = await request(app).get('/api/matchmaking/search').expect(401);
  ...
  describe('GET /api/matchmaking/project/:id/matches', () => {
    it('should return matches for authenticated user', async () => {
      ...
      mockPoolQuery.mockResolvedValueOnce({
        rows: [
          { project_id: 'proj-001', gps_location: 'POINT(37.1 36.2)', damage_type: 'structural' },
        ],
        rowCount: 1,
      });
      // Second query: matched engineers
      mockPoolQuery.mockResolvedValueOnce({
        rows: [{ user_id: 'eng-1', full_name: 'Eng A', dynamic_score: 90 }],
        rowCount: 1,
      });

      const res = await request(app).get('/api/matchmaking/project/proj-001/matches').expect(200);
    ...
    it('should create bid with valid data via transaction', async () => {
      const mockClient = {
        query: vi
          ...
          // 3. existingAsContractor check
          .mockResolvedValueOnce({ rows: [], rowCount: 0 })
          // 4. duplicate check (no existing bid)
          .mockResolvedValueOnce({ rows: [], rowCount: 0 })
          // 5. INSERT bid
    ...
    it('should return 409 for duplicate bid', async () => {
      const mockClient = {
        query: vi
          ...
          // 3. existingAsContractor check
          .mockResolvedValueOnce({ rows: [], rowCount: 0 })
          // 4. duplicate found (active bid exists)
  ...
  describe('GET /api/matchmaking/project/:id/bids', () => {
    it('should return bids for homeowner', async () => {
      ...
      const res = await request(app).get('/api/matchmaking/project/proj-001/bids').expect(200);
    ...
    it('should reject engineer from viewing bids', async () => {
      const res = await request(app).get('/api/matchmaking/project/proj-001/bids').expect(403);
  ...
  describe('POST /api/matchmaking/bids/:bidId/accept', () => {
    it('should accept bid as homeowner', async () => {
      ...
      const res = await request(app).post('/api/matchmaking/bids/bid-001/accept').expect(200);
  ...
  describe('GET /api/matchmaking/engineer/:id/score', () => {
    it('should return engineer score breakdown', async () => {
      ...
      const res = await request(app).get('/api/matchmaking/engineer/eng-001/score').expect(200);
    ...
    it('should return 404 for non-existent engineer', async () => {
      ...
      const res = await request(app).get('/api/matchmaking/engineer/nonexistent/score').expect(404);
