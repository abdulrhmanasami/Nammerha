...
import type {
    ReviewableType, ReviewStatus,
    CreateReviewDTO, UpdateReviewDTO, CreateResponseDTO, FlagReviewDTO,
    ApiResponse,
} from '../types';
...
const PROFILE_TABLE_WHITELIST: ReadonlySet<string> = new Set([
    'contractor_profiles', 'supplier_profiles', 'engineer_profiles',
    'tradesperson_profiles', 'homeowner_profiles',
]);
...
router.get('/:type/:id', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const orderClause = sort === 'helpful' ? 'r.helpful_count DESC, r.created_at DESC'
            : sort === 'highest' ? 'r.overall_rating DESC, r.created_at DESC'
            : sort === 'lowest'  ? 'r.overall_rating ASC, r.created_at DESC'
            : 'r.created_at DESC';

        // Fetch reviews with reviewer info
        const reviewsResult = await query<{
            review_id: string; reviewer_id: string; reviewable_type: string;
            reviewable_id: string; project_id: string | null;
            overall_rating: number; title: string | null; body: string;
            is_verified_interaction: boolean; status: ReviewStatus;
            edit_count: number; edited_at: Date | null;
            helpful_count: number; created_at: Date;
            reviewer_name: string; reviewer_avatar_url: string | null;
        }>(
        ...
        const ratingsMap: Record<string, Array<{ dimension_key: string; label_en: string; label_ar: string; score: number }>> = {};

        if (reviewIds.length > 0) {
            const ratingsResult = await query<{
                review_id: string; dimension_key: string;
                label_en: string; label_ar: string; score: number;
            }>(
            ...
            for (const row of ratingsResult.rows) {
                ...
                if (!list) { ratingsMap[row.review_id] = []; }
        ...
        const responsesMap: Record<string, { response_id: string; body: string; created_at: Date; responder_name: string }> = {};

        if (reviewIds.length > 0) {
            const responsesResult = await query<{
                review_id: string; response_id: string; body: string;
                created_at: Date; responder_name: string;
            }>(
...
router.get('/aggregates/:type/:id', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const aggResult = await query<{
            ...
            verified_reviews: number; dimension_averages: Record<string, number>;
            rating_distribution: Record<string, number>; trust_score: string;
        ...
        const dimsResult = await query<{
            dimension_key: string; label_en: string; label_ar: string; weight: string;
        }>(
...
router.post('/', authMiddleware, requireActive, async (req: Request, res: Response): Promise<void> => {
    try {
        if (!req.authUser) {
        ...
        if (!dto.reviewable_id || !dto.body || !dto.overall_rating) {
            res.status(400).json({ success: false, error: 'Missing required fields: reviewable_id, body, overall_rating' } as ApiResponse);
            return;
        }

        if (dto.overall_rating < 1 || dto.overall_rating > 5 || !Number.isInteger(dto.overall_rating)) {
            res.status(400).json({ success: false, error: 'overall_rating must be an integer between 1 and 5' } as ApiResponse);
            return;
        }
        ...
        if (dto.reviewable_type !== 'project') {
            ...
            if (!PROFILE_TABLE_WHITELIST.has(dto.reviewable_type)) {
                res.status(400).json({ success: false, error: 'Invalid profile type for review' } as ApiResponse);
                return;
            }
        ...
        if (dto.project_id && dto.reviewable_type !== 'project') {
            ...
            const interactionCheck = await query<{ found: boolean }>(
                `SELECT EXISTS(
                    ...
                    WHERE project_id = $1 AND donor_id = $2 AND payment_status IN ('released', 'locked')
                    UNION ALL
            ...
            if (interactionCheck.rows[0]?.found) {
                isVerified = true;
                verificationContext = { project_id: dto.project_id, verified_at: new Date().toISOString() };
            }
        } else if (dto.reviewable_type === 'project' && dto.reviewable_id) {
            // Donor reviewing a project — check they funded it
            ...
            if (fundCheck.rows[0]?.found) {
                isVerified = true;
                verificationContext = { project_id: dto.reviewable_id, verified_at: new Date().toISOString() };
            }
        }
...
router.put('/:reviewId', authMiddleware, requireActive, async (req: Request, res: Response): Promise<void> => {
    try {
        if (!req.authUser) {
            res.status(401).json({ success: false, error: 'Authentication required' } as ApiResponse);
            return;
        ...
        const existing = await query<{
            review_id: string; reviewer_id: string; edit_count: number;
            created_at: Date; status: ReviewStatus;
        }>(
        ...
        if (review.reviewer_id !== req.authUser.user_id) {
            res.status(403).json({ success: false, error: 'You can only edit your own reviews' } as ApiResponse);
            return;
        }
        ...
        if (elapsed > EDIT_WINDOW_MS) {
            res.status(403).json({ success: false, error: 'Edit window has expired (48 hours)' } as ApiResponse);
            return;
        }

        // Edit count limit
        if (review.edit_count >= MAX_EDITS) {
            res.status(403).json({ success: false, error: `Maximum ${MAX_EDITS} edits allowed` } as ApiResponse);
            return;
        }
        ...
        if (dto.overall_rating !== undefined) {
            if (dto.overall_rating < 1 || dto.overall_rating > 5 || !Number.isInteger(dto.overall_rating)) {
                res.status(400).json({ success: false, error: 'overall_rating must be integer 1-5' } as ApiResponse);
                return;
            }
        }
        ...
        await transaction(async (client) => {
            ...
            if (dto.ratings && dto.ratings.length > 0) {
                ...
                const typeResult = await client.query<{ reviewable_type: string }>(
                    `SELECT reviewable_type FROM reviews WHERE review_id = $1`, [reviewId]
                );
...
router.delete('/:reviewId', authMiddleware, requireActive, async (req: Request, res: Response): Promise<void> => {
    try {
        if (!req.authUser) {
            res.status(401).json({ success: false, error: 'Authentication required' } as ApiResponse);
            return;
        ...
        if (result.rowCount === 0) {
            res.status(404).json({ success: false, error: 'Review not found or already removed' } as ApiResponse);
            return;
        }
...
router.post('/:reviewId/response', authMiddleware, requireActive, async (req: Request, res: Response): Promise<void> => {
    try {
        if (!req.authUser) {
            res.status(401).json({ success: false, error: 'Authentication required' } as ApiResponse);
            return;
        ...
        const reviewResult = await query<{
            reviewable_type: string; reviewable_id: string;
        }>(
        ...
        if (review.reviewable_type === 'project') {
            const projCheck = await query<{ homeowner_id: string }>(
                `SELECT homeowner_id FROM projects WHERE project_id = $1`, [review.reviewable_id]
            );
        ...
        if (!isOwner) {
            res.status(403).json({ success: false, error: 'Only the reviewed party can respond' } as ApiResponse);
            return;
        }
        ...
        logger.info('Review response added', { review_id: reviewId, responder: req.authUser.user_id });
        ...
        if (error instanceof Error && error.message.includes('unique constraint')) {
            res.status(409).json({ success: false, error: 'Response already exists for this review' } as ApiResponse);
            return;
        }
...
router.post('/:reviewId/flag', authMiddleware, requireActive, async (req: Request, res: Response): Promise<void> => {
    try {
        if (!req.authUser) {
            res.status(401).json({ success: false, error: 'Authentication required' } as ApiResponse);
            return;
        ...
        const validReasons: ReadonlySet<string> = new Set(['spam', 'inappropriate', 'fake', 'conflict_of_interest', 'other']);
        ...
        const reviewCheck = await query<{ review_id: string }>(
            `SELECT review_id FROM reviews WHERE review_id = $1`, [reviewId]
        );
        ...
        logger.info('Review flagged', { review_id: reviewId, reporter: req.authUser.user_id, reason: dto.reason });

        res.status(201).json({ success: true, message: 'Report submitted' } as ApiResponse);
    } catch (error) {
        if (error instanceof Error && error.message.includes('unique constraint')) {
            res.status(409).json({ success: false, error: 'You have already reported this review' } as ApiResponse);
            return;
        }
...
router.post('/:reviewId/helpful', authMiddleware, async (req: Request, res: Response): Promise<void> => {
    try {
        if (!req.authUser) {
            res.status(401).json({ success: false, error: 'Authentication required' } as ApiResponse);
            return;
        ...
        if (typeof is_helpful !== 'boolean') {
            res.status(400).json({ success: false, error: 'is_helpful must be a boolean' } as ApiResponse);
            return;
        }
...
router.get('/my-reviews', authMiddleware, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const result = await query<{
            review_id: string; reviewable_type: string; reviewable_id: string;
            project_id: string | null; overall_rating: number; title: string | null;
            body: string; status: ReviewStatus; is_verified_interaction: boolean;
            helpful_count: number; edit_count: number; created_at: Date; edited_at: Date | null;
        }>(
