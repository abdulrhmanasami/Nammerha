...
// ============================================================================
// REST API for user impact message management.
//
// Endpoints:
...
// All impact routes require: JWT + active + user role
...
router.get('/messages', async (req: Request, res: Response) => {
  try {
    ...
    const messages = await impactService.getUserMessages(userId, {
      limit,
      offset,
      unreadOnly,
    });
...
router.put('/messages/:id/read', async (req: Request, res: Response) => {
  try {
    ...
    const updated = await impactService.markAsRead(messageId, userId);
