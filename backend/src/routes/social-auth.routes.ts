...
async function verifyFacebookToken(accessToken: string): Promise<VerifiedSocialUser> {
  if (!FACEBOOK_APP_ID || !FACEBOOK_APP_SECRET) {
    throw new Error(
      'Facebook Login is not configured. Set FACEBOOK_APP_ID and FACEBOOK_APP_SECRET in environment.',
    );
  }
 ...
 */
router.post('/social', socialAuthLimiter, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    let userId: string;
    // P1-AUTH-004 FIX: Track if this is a new user registration.
    // Frontend uses this to trigger onboarding welcome tour.
    let isNewUser = false;
    ...
    if (existingLink.rows[0]) {
      ...
      if (existingUser.rows[0]) {
        ...
        logger.info('Social auth: linked to existing account', {
          provider,
          userId,
          email: socialUser.email,
        });
    ...
    res.status(200).json({
      ...
      data: {
        ...
        // P1-AUTH-004 FIX: Frontend uses this to trigger onboarding.
