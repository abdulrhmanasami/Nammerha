...
async function enterpriseApiAuth(req: Request, res: Response, next: () => void): Promise<void> {
  ...
  try {
    ...
    // CRIT-06 FIX: Defer API call logging to capture actual response status code.
    // Previously hardcoded 200 before route handler executed.
    ...
    logger.error('Enterprise API auth failed', {
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { org_name, org_type, contact_email, tier, annual_fee_cents } = createOrgSchema.parse(
        req.body,
      );
