...
router.get('/refund-requests', requireRole('admin'), async (_req: Request, res: Response) => {
  try {
...
router.get('/kyc/queue', requireRole('admin'), async (req: Request, res: Response) => {
  try {
...
router.get('/kyc/stats', requireRole('admin'), async (_req: Request, res: Response) => {
  try {
...
router.post('/kyc/:userId/decision', requireRole('admin'), async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof ZodError) {
      res
        .status(400)
        .json({ success: false, error: 'Validation failed', details: error.issues } as ApiResponse);
      return;
    }
