...
export const projectMutationResolvers = {
  createProject: async (
    ...
    const result = await dbQuery(
      ...
      [
        user.user_id,
        input.title.trim(),
        input.damageType.toLowerCase(),
        input.damageSeverity?.toLowerCase() ?? null,
        input.description?.trim() ?? null,
        input.gpsLng,
        input.gpsLat,
        input.addressText?.trim() ?? null,
  ...
  addBOQItem: async (
    ...
    const row = result.rows[0] as Record<string, unknown>;
    if (!row) {
      throw new Error('Failed to add BOQ item');
    }
...
export const spatialProofMutationResolvers = {
  submitSpatialProof: async (
    ...
    const proof = await executionService.submitSpatialProof(user.user_id, {
...
export const supplierMutationResolvers = {
  acknowledgePO: async (_: unknown, args: { poId: string }, context: GQLContext) => {
...
export const reviewMutationResolvers = {
  createReview: async (
    ...
    const result = await dbQuery(
      ...
      [
        user.user_id,
        input.reviewableType.toLowerCase(),
        input.reviewableId,
